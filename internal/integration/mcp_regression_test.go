...
func buildRegressionUpstreamResponse(t testing.TB, content string) *mcp.Message {
	t.Helper()
	...
	rawBytes, err := json.Marshal(rawMsg)
	if err != nil {
		t.Fatalf("json.Marshal rawMsg: %v", err)
	}
...
func buildRegressionToolListResponse(tb testing.TB, tools []string) *mcp.Message {
	tb.Helper()
	toolList := make([]map[string]interface{}, len(tools))
	for i, tl := range tools {
		toolList[i] = map[string]interface{}{
			"name":        tl,
			"description": "Test tool " + tl,
	...
	resultJSON, err := json.Marshal(result)
	if err != nil {
		tb.Fatalf("json.Marshal result: %v", err)
	}
	...
	rawBytes, err := json.Marshal(rawMsg)
	if err != nil {
		tb.Fatalf("json.Marshal rawMsg: %v", err)
	}
...
func buildRegressionMessage(t testing.TB, method string, id float64, params map[string]interface{}, sess *session.Session) *mcp.Message {
	t.Helper()
...
func buildRegressionChain(
	...
	// PolicyActionInterceptor -> ResponseScanInterceptor
	policyInterceptor := action.NewPolicyActionInterceptor(policyEngine, responseScan, logger)
...
func (r *regressionStatsRecorder) RecordBlocked()           { r.denies++ }
func (r *regressionStatsRecorder) RecordRateLimited()       {}
func (r *regressionStatsRecorder) RecordWarned()            {}
...
func TestMCPRegression_ExistingTestsUnmodified(t *testing.T) {
	...
	upstream := &mockUpstreamRouter{
		toolCallResponse: buildRegressionUpstreamResponse(t, "File contents: hello regression test"),
		toolListResponse: buildRegressionToolListResponse(t, []string{"read_file", "write_file", "list_files"}),
	}
	...
	t.Run("NonToolCallPassthrough", func(t *testing.T) {
		...
		notification := buildRegressionMessage(t, "notifications/initialized", 100, nil, sess)
	...
	t.Run("ToolsListAggregation", func(t *testing.T) {
		...
		msg := buildRegressionMessage(t, "tools/list", 4, nil, sess)
