...
// perfQuarantineChecker is a no-op quarantine checker for benchmarks.
type perfQuarantineChecker struct{}

func (p *perfQuarantineChecker) IsQuarantined(_ string) bool { return false }

// buildPerfChain creates the full ActionInterceptor chain matching
// the real boot_interceptors.go order (inner to outer):
// Quarantine -> Policy -> Approval -> Outbound -> Transform -> ContentScan -> ResponseScan -> terminal
func buildPerfChain(t testing.TB, policyEngine policy.PolicyEngine) action.ActionInterceptor {
	...
	// ContentScanInterceptor (enabled, no whitelist, no event bus)
	contentScanner := action.NewContentScanner()
	contentScan := action.NewContentScanInterceptor(contentScanner, responseScan, true, logger)

	// TransformInterceptor (empty store — no rules to apply)
	transformStore := transform.NewMemoryTransformStore()
	transformExecutor := transform.NewTransformExecutor(logger)
	transformInterceptor := transform.NewTransformInterceptor(transformStore, transformExecutor, contentScan, logger)

	// ApprovalInterceptor (empty store — no approvals pending)
	approvalStore := action.NewApprovalStore(100)
	approval := action.NewApprovalInterceptor(approvalStore, transformInterceptor, logger)

	// PolicyActionInterceptor
	policyInterceptor := action.NewPolicyActionInterceptor(policyEngine, approval, logger)

	// QuarantineInterceptor (nothing quarantined)
	quarantine := action.NewQuarantineInterceptor(&perfQuarantineChecker{}, policyInterceptor, logger)
...
func (p *perfStatsRecorder) RecordBlocked()           {}
func (p *perfStatsRecorder) RecordRateLimited()       {}
func (p *perfStatsRecorder) RecordWarned()            {}
