...
// mockStatsRecorder tracks allow/deny/rate-limit/warned counts.
...
func (m *mockStatsRecorder) RecordBlocked()     { m.denies++ }
func (m *mockStatsRecorder) RecordRateLimited() { m.rateLimited++ }
func (m *mockStatsRecorder) RecordWarned()      { m.warned++ }
...
func newTestToolCallMessage(t testing.TB, toolName string, args map[string]interface{}, sess *session.Session) *mcp.Message {
	t.Helper()
...
func newTestServerResponse(t testing.TB) *mcp.Message {
	t.Helper()
...
func TestMCPFullPath_AllowedToolCall(t *testing.T) {
	...
	serverResponse := newTestServerResponse(t)
	...
	policyInterceptor := action.NewPolicyActionInterceptor(engine, responseScanInterceptor, logger)
	...
	sess := newTestSession()
	toolCallMsg := newTestToolCallMessage(t, "read_file", map[string]interface{}{"path": "/tmp/data.txt"}, sess)
...
func TestMCPFullPath_DeniedToolCall(t *testing.T) {
	...
	sess := newTestSession()
	toolCallMsg := newTestToolCallMessage(t, "exec_command", map[string]interface{}{"cmd": "rm -rf /"}, sess)
