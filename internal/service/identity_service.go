...
func (s *IdentityService) GenerateKey(_ context.Context, input GenerateKeyInput) (*GenerateKeyResult, error) {
	...
	// Store the first 8 chars of the cleartext key as a prefix.
	// This enables the fast-path Argon2id lookup in APIKeyService.Validate()
	// without O(n) iteration across all keys.
	keyPrefix := ""
