...
// mockNilInterceptor returns (nil, nil) to silently drop messages.
type mockNilInterceptor struct{}

func (m *mockNilInterceptor) Intercept(ctx context.Context, msg *mcp.Message) (*mcp.Message, error) {
	return nil, nil
}

// TestProxyService_NilInterceptorResult verifies that ProxyService does not panic
// when the interceptor returns (nil, nil), which means "silently drop the message".
func TestProxyService_NilInterceptorResult(t *testing.T) {
	defer goleak.VerifyNone(t)

	serverInReader, serverInWriter := io.Pipe()
	serverOutReader, serverOutWriter := io.Pipe()

	// Server: just drain and close when stdin closes
	serverDone := make(chan struct{})
	go func() {
		defer close(serverDone)
		defer func() { _ = serverOutWriter.Close() }()
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	proxyService := NewProxyService(mockClient, &mockNilInterceptor{}, logger)
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- proxyService.Run(ctx, clientInReader, clientOut)
	}()

	// Give proxy time to start
	time.Sleep(50 * time.Millisecond)

	// Send a message — interceptor will return (nil, nil)
	_, err := clientInWriter.Write([]byte(`{"jsonrpc":"2.0","method":"test","id":1}` + "\n"))
	if err != nil {
		t.Fatalf("write failed: %v", err)
	}

	// Give proxy time to process the message (no panic = success)
	time.Sleep(100 * time.Millisecond)
	...
	// Verify no output was written to client (message was silently dropped)
	if clientOut.Len() != 0 {
		t.Errorf("expected no output (message dropped), got: %s", clientOut.String())
	}
