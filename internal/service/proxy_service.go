...
func (p *ProxyService) Run(ctx context.Context, clientIn io.Reader, clientOut io.Writer) error {
	...
	if waitErr := p.client.Wait(); waitErr != nil {
		if parentCtx.Err() != nil {
			// Context was externally cancelled; process exit is expected.
			return parentCtx.Err()
		}
		// Normal termination but upstream exited with an error (e.g. non-zero
		// exit code). Propagate so the caller can surface it.
		logger.Warn("upstream server exited with error", "error", waitErr)
		return fmt.Errorf("upstream server: %w", waitErr)
	}

	// Return parent context error only if external cancellation occurred.
	// If termination was normal (we called cancel() ourselves), parentCtx.Err()
	// will be nil.
	return parentCtx.Err()
}
...
func (p *ProxyService) copyMessages(ctx context.Context, src io.Reader, dst io.Writer, clientOut io.Writer, direction mcp.Direction, logger *slog.Logger) error {
	...
	// Note: upstream_manager.go uses json.Decoder for its long-lived reader
	// goroutine to handle embedded newlines. Here we keep bufio.Scanner because
	// copyMessages is typically fed from bytes.Reader (HTTP body) or short-lived
	// pipes where messages are well-formed single-line JSON.
	...
	for scanner.Scan() {
		...
		// Attempt to decode for inspection
		if decoded, err := mcp.DecodeMessage(raw); err == nil {
			msg.Decoded = decoded

			// MED-9: Drop tools/call notifications (no ID) from client.
			// In single-upstream mode these would be forwarded and executed
			// without any response tracking — silently drop them.
			if direction == mcp.ClientToServer {
				if req := msg.Request(); req != nil && !req.ID.IsValid() && req.Method == "tools/call" {
					logger.Warn("dropping tools/call notification (no id)")
					continue
				}
				_ = msg.ParseParams() // Ignore error, ParsedParams will be nil if fails
			}
		} else {
			// Malformed JSON from client: reject immediately with JSON-RPC parse error (-32700)
			if direction == mcp.ClientToServer && clientOut != nil {
				logger.Warn("malformed JSON-RPC from client, rejecting",
					"error", err,
				)
				errResp := proxy.CreateJSONRPCError(nil, -32700, "Parse error")
				_, _ = clientOut.Write(errResp)
				_, _ = clientOut.Write([]byte("\n"))
				continue
			}
			// Server→client: log and pass through raw (best-effort)
			logger.Debug("failed to decode server message, passing through raw",
				"error", err,
			)
		}
		...
		// Guard: interceptor may return (nil, nil) to silently drop a message.
		if processedMsg == nil {
			logger.Debug("interceptor dropped message (nil, nil)", "direction", direction)
			continue
		}
