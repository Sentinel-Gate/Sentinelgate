...
func TestPolicyAdminService_Create(t *testing.T) {
	...
	p := &policy.Policy{
		...
		Priority:    10,
		Enabled:     true, // M-8: caller explicitly enables
		Rules: []policy.Rule{
			{
				Name:      "allow-read",
				Priority:  100,
	...
	// M-8: Must respect caller's Enabled value.
	if !created.Enabled {
		t.Error("Create() should respect Enabled=true from caller")
	}
...
func TestPolicyAdminService_Create_TriggersReload(t *testing.T) {
	...
	// Initial evaluation with default policy - should be allowed (default allow).
