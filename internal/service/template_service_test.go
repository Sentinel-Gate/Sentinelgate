...
func TestTemplateService_Apply_Success(t *testing.T) {
	...
	if created.Name != "File Server — Read Only" {
		t.Errorf("Apply().Name = %q, want %q", created.Name, "File Server — Read Only")
	}
	...
	if got.Name != "File Server — Read Only" {
		t.Errorf("Stored policy Name = %q, want %q", got.Name, "File Server — Read Only")
	}
}
