...
func TestPolicyServiceBasicEvaluation(t *testing.T) {
	...
	svc, err := NewPolicyService(store, logger)
	if err != nil {
...
func TestPolicyService_CacheBounded(t *testing.T) {
	...
	svc, err := NewPolicyService(store, logger, WithCacheSize(10))
	if err != nil {
		t.Fatalf("NewPolicyService failed: %v", err)
	}
...
func TestComputeCacheKey_Deterministic(t *testing.T) {
	...
	key1 := computeCacheKey("read_file", []string{"user", "admin"}, map[string]interface{}{"path": "/tmp"})
	key2 := computeCacheKey("read_file", []string{"admin", "user"}, map[string]interface{}{"path": "/tmp"})
	...
	key3 := computeCacheKey("write_file", []string{"user"}, nil)
	if key1 == key3 {
		t.Error("different inputs should produce different cache keys")
	}

	// Different args should produce different keys
	key4 := computeCacheKey("read_file", []string{"user", "admin"}, map[string]interface{}{"path": "/etc"})
	if key1 == key4 {
	...
	key5 := computeCacheKey("test_tool", []string{"user"}, nil)
	key6 := computeCacheKey("test_tool", []string{"user"}, map[string]interface{}{})
	if key5 != key6 {
