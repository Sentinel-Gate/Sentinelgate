...
type CompiledRule struct {
	...
	Condition       string      // Original CEL condition text (empty means unconditional)
...
func computeCacheKey(evalCtx policy.EvaluationContext) (uint64, bool) {
	h := xxhash.New()
	...
	_, _ = h.WriteString(evalCtx.ActionType)
	_, _ = h.Write([]byte{0})
	_, _ = h.WriteString(evalCtx.Protocol)
	_, _ = h.Write([]byte{0})
	_, _ = h.WriteString(evalCtx.Framework)
	_, _ = h.Write([]byte{0})

	// Destination fields (policies can condition on destination)
	_, _ = h.WriteString(evalCtx.DestURL)
	_, _ = h.Write([]byte{0})
	_, _ = h.WriteString(evalCtx.DestDomain)
	_, _ = h.Write([]byte{0})
	_, _ = h.WriteString(evalCtx.DestIP)
	_, _ = h.Write([]byte{0})
	_, _ = fmt.Fprintf(h, "%d", evalCtx.DestPort)
	_, _ = h.Write([]byte{0})
	_, _ = h.WriteString(evalCtx.DestScheme)
	_, _ = h.Write([]byte{0})
	_, _ = h.WriteString(evalCtx.DestPath)
	_, _ = h.Write([]byte{0})
	_, _ = h.WriteString(evalCtx.DestCommand)
	_, _ = h.Write([]byte{0})

	// Gateway
	_, _ = h.WriteString(evalCtx.Gateway)
	_, _ = h.Write([]byte{0})

	// H-2: FrameworkAttrs (sorted keys + values for determinism)
	if len(evalCtx.FrameworkAttrs) > 0 {
		fKeys := make([]string, 0, len(evalCtx.FrameworkAttrs))
		for k := range evalCtx.FrameworkAttrs {
			fKeys = append(fKeys, k)
		}
		sort.Strings(fKeys)
		for _, k := range fKeys {
			_, _ = h.WriteString(k)
			_, _ = h.Write([]byte{0x1F}) // unit separator between key and value
			_, _ = h.WriteString(evalCtx.FrameworkAttrs[k])
			_, _ = h.Write([]byte{0x1E}) // record separator between entries
		}
	}
	_, _ = h.Write([]byte{0})
	...
	if len(evalCtx.ToolArguments) > 0 {
		argsJSON, err := json.Marshal(evalCtx.ToolArguments)
		if err != nil {
			return 0, false
		}
...
// CELEvaluator returns the CEL evaluator for use by other services (e.g., simulation).
func (s *PolicyService) CELEvaluator() *celeval.Evaluator {
	return s.evaluator
}
...
func (s *PolicyService) compileRules(rules []policy.Rule) ([]CompiledRule, error) {
	...
	for _, rule := range rules {
		...
		prg, err := s.evaluator.Compile(cond)
		if err != nil {
...
func (s *PolicyService) loadSnapshot() *CompiledRulesSnapshot {
	snap, ok := s.snapshot.Load().(*CompiledRulesSnapshot)
	if !ok {
		return nil
	}
	return snap
...
// For namespaced tools (e.g. "desktop/read_file"), it also includes rules
// that match the bare name ("read_file") for backward compatibility.
func (s *PolicyService) getCandidateRules(idx *RuleIndex, toolName string) []CompiledRule {
	// Get exact matches for the full resolved name
	exact := idx.Exact[toolName]

	// Backward compatibility: if the tool name is namespaced (contains "/"),
	// also include rules that match the bare name part.
	// This ensures that a rule with tool_match="read_file" still applies
	// to "desktop/read_file" after namespacing is enabled.
	if slashIdx := strings.Index(toolName, "/"); slashIdx >= 0 {
		bareName := toolName[slashIdx+1:]
		if bareRules := idx.Exact[bareName]; len(bareRules) > 0 {
			// Merge bare-name rules into exact, maintaining priority order
			merged := make([]CompiledRule, 0, len(exact)+len(bareRules))
			i, j := 0, 0
			for i < len(exact) && j < len(bareRules) {
				if exact[i].Priority >= bareRules[j].Priority {
					merged = append(merged, exact[i])
					i++
				} else {
					merged = append(merged, bareRules[j])
					j++
				}
			}
			merged = append(merged, exact[i:]...)
...
func (s *PolicyService) Evaluate(ctx context.Context, evalCtx policy.EvaluationContext) (policy.Decision, error) {
	...
	cacheKey, cacheKeyValid := computeCacheKey(evalCtx)

	// Check cache first (hot path optimization).
	// Skip cache for playground/test evaluations and session-aware contexts
	// where results depend on dynamic session state.
	// Also skip if cache key computation failed (non-marshalable args).
	// M-12: Also skip cache when session usage counters are non-zero,
	// since CEL rules like "session_call_count > 100" depend on dynamic state.
	hasSessionCounters := evalCtx.SessionCallCount > 0 || evalCtx.SessionWriteCount > 0
	useCache := !evalCtx.SkipCache && cacheKeyValid && len(evalCtx.SessionActionHistory) == 0 && !hasSessionCounters
	if useCache {
	...
	if snapshot == nil {
		// M-7: Fail-closed when policy engine is not ready (startup race / load failure).
		return policy.Decision{Allowed: false, Reason: "policy engine not ready"}, nil
	}
	...
	for _, rule := range candidates {
		...
		if strings.ContainsAny(rule.ToolMatch, "*?[") {
			...
			if rule.ToolMatch != "*" {
				...
				// M-11: filepath.Match "*" does not cross "." separators in tool names
				// (e.g., "mcp.*" won't match "mcp.server.read"). As a workaround,
				// also try matching with "/" as separator since filepath.Match handles "/".
				if !matched && strings.Contains(evalCtx.ToolName, ".") {
					matched, _ = filepath.Match(
						strings.ReplaceAll(rule.ToolMatch, ".", "/"),
						strings.ReplaceAll(evalCtx.ToolName, ".", "/"),
					)
				}
				// Backward compat: if the pattern has no "/" (bare pattern like "read_*"),
				// also try matching against just the bare part of a namespaced tool name.
				// This ensures "read_*" matches "desktop/read_file".
				if !matched && !strings.Contains(rule.ToolMatch, "/") {
					if slashIdx := strings.Index(evalCtx.ToolName, "/"); slashIdx >= 0 {
						barePart := evalCtx.ToolName[slashIdx+1:]
						matched, _ = filepath.Match(rule.ToolMatch, barePart)
					}
				}
				if !matched {
					continue
		...
		result, err := s.evaluator.Evaluate(ctx, rule.Program, evalCtx)
		if err != nil {
...
// GetMatchingRules returns all compiled rules whose tool_match pattern matches
// the given tool name, without evaluating CEL conditions.
// Used to determine if a tool has mixed-action rules (conditional status).
func (s *PolicyService) GetMatchingRules(toolName string) []CompiledRule {
	snapshot := s.loadSnapshot()
	if snapshot == nil {
		return nil
	}

	candidates := s.getCandidateRules(snapshot.Index, toolName)
	...
	for _, rule := range candidates {
		if strings.ContainsAny(rule.ToolMatch, "*?[") {
			...
			ok, err := filepath.Match(rule.ToolMatch, toolName)
			if err != nil {
				s.logger.Warn("invalid glob pattern", "rule", rule.ID, "pattern", rule.ToolMatch, "error", err)
				continue
			}
			...
			if !ok && !strings.Contains(rule.ToolMatch, "/") {
				if slashIdx := strings.Index(toolName, "/"); slashIdx >= 0 {
					barePart := toolName[slashIdx+1:]
					ok, _ = filepath.Match(rule.ToolMatch, barePart)
				}
			}
			if !ok {
				continue
