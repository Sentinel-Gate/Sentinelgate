...
type mgrMockMCPClient struct {
	...
	startCount   int
	closeCount   int
	simulateHTTP bool // if true, Start returns pipes with auto-responder for init handshake
	pipeCleanup  func()
	pipeDone     chan struct{} // closed when auto-responder goroutine exits
}
...
func (m *mgrMockMCPClient) Start(ctx context.Context) (io.WriteCloser, io.ReadCloser, error) {
	...
	// Use pipes with an auto-responder for initialize handshake.
	// All upstream types now require the init handshake.
	...
	go func() {
		...
		for scanner.Scan() {
			line := scanner.Text()
			// Respond to initialize with a valid JSON-RPC response
			if strings.Contains(line, "initialize") && strings.Contains(line, "\"id\"") {
				...
				if json.Unmarshal([]byte(line), &req) == nil && req.ID != nil {
					resp := fmt.Sprintf(`{"jsonrpc":"2.0","id":%s,"result":{"protocolVersion":"2025-06-18","capabilities":{"tools":{}},"serverInfo":{"name":"mock","version":"1.0"}}}`, string(req.ID))
	...
	m.pipeDone = done
	// Close both pipe ends on mock Close to terminate all goroutines:
	// - reqReader.Close() unblocks the auto-responder scanner
	// - respReader.Close() unblocks the attemptConnect channel reader scanner
	m.pipeCleanup = func() {
		reqReader.Close()
		respReader.Close()
	}
...
func (m *mgrMockMCPClient) Close() error {
	...
	// Clean up pipes and wait for auto-responder goroutine to exit.
	// Must be done outside the lock to avoid deadlock with goroutine exit.
	if cleanup != nil {
		cleanup()
	}
	if done != nil {
...
// It closes pipes to unblock reader goroutines and signals Wait() to return.
func (m *mgrMockMCPClient) simulateCrash() {
	...
	// Close pipes to terminate auto-responder goroutine
	if cleanup != nil {
		cleanup()
	}
	if done != nil {
...
func testManagerEnv(t *testing.T, upstreams ...*upstream.Upstream) (*UpstreamManager, map[string]*mgrMockMCPClient) {
	...
	factory := func(u *upstream.Upstream) (outbound.MCPClient, error) {
		...
		if _, ok := mockClients[u.ID]; ok {
			// For reconnection, create fresh mock but preserve simulateHTTP
			fresh := newMgrMockMCPClient()
			fresh.simulateHTTP = (u.Type == upstream.UpstreamTypeHTTP)
			mockClients[u.ID] = fresh
			return fresh, nil
		}
...
func TestUpstreamManager_StabilityReset(t *testing.T) {
	...
	// NOTE: Intentionally accessing unexported fields (mgr.connections, conn.retryCount,
	// conn.mu) to manipulate internal state — this is an internal (same-package) test.
	mgr.mu.RLock()
...
// TestSetGlobalRetryConfig verifies that SetGlobalRetryConfig updates the
// manager-level retry limits that are consulted during backoff scheduling.
func TestSetGlobalRetryConfig(t *testing.T) {
	mgr, _ := testManagerEnv(t)
	defer goleak.VerifyNone(t)
	defer func() { _ = mgr.Close() }()

	// Default values
	if mgr.maxRetries != 10 {
		t.Errorf("default maxRetries = %d, want 10", mgr.maxRetries)
	}
	if mgr.backoffCap != 60*time.Second {
		t.Errorf("default backoffCap = %v, want 60s", mgr.backoffCap)
	}

	// Update config
	mgr.SetGlobalRetryConfig(3, 5*time.Second)

	mgr.mu.RLock()
	gotMax := mgr.maxRetries
	gotCap := mgr.backoffCap
	mgr.mu.RUnlock()

	if gotMax != 3 {
		t.Errorf("maxRetries after SetGlobalRetryConfig = %d, want 3", gotMax)
	}
	if gotCap != 5*time.Second {
		t.Errorf("backoffCap after SetGlobalRetryConfig = %v, want 5s", gotCap)
	}
}

// TestResetRetryCount verifies that ResetRetryCount sets a managed upstream's
// retry counter back to zero.
func TestResetRetryCount(t *testing.T) {
	...
	ctx := context.Background()
	if err := mgr.Start(ctx, "up-retry"); err != nil {
		t.Fatalf("Start() unexpected error: %v", err)
	}
	time.Sleep(50 * time.Millisecond)

	// Manually set retry count on the connection.
	// NOTE: Intentionally accessing unexported fields (mgr.connections, conn.retryCount,
	// conn.mu) to manipulate internal state — this is an internal (same-package) test.
	mgr.mu.RLock()
	conn := mgr.connections["up-retry"]
	mgr.mu.RUnlock()
	...
	conn.mu.Unlock()
	if rc != 7 {
		t.Fatalf("retryCount before reset = %d, want 7", rc)
	}

	// Reset it.
	if err := mgr.ResetRetryCount("up-retry"); err != nil {
		t.Fatalf("ResetRetryCount() unexpected error: %v", err)
	}
	...
	conn.mu.Unlock()
	if rc != 0 {
		t.Errorf("retryCount after ResetRetryCount = %d, want 0", rc)
	}
}

// TestResetRetryCount_NotManaged verifies that ResetRetryCount returns an error
// for upstreams that are not currently managed by the manager.
func TestResetRetryCount_NotManaged(t *testing.T) {
	mgr, _ := testManagerEnv(t)
	defer goleak.VerifyNone(t)
	defer func() { _ = mgr.Close() }()

	err := mgr.ResetRetryCount("nonexistent-upstream")
	if err == nil {
		t.Fatal("ResetRetryCount() for unmanaged upstream should return error")
	}
}
