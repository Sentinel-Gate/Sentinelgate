...
// --- PostMutationHook Tests ---

// TestIdentityService_PostMutationHook_GenerateKey verifies that PostMutationHook
// is called after a successful GenerateKey operation. This covers the centralized
// auth store sync pattern (HARD-08).
func TestIdentityService_PostMutationHook_GenerateKey(t *testing.T) {
	svc, _, _ := testIdentityEnv(t)
	ctx := context.Background()

	var hookCallCount int
	svc.SetPostMutationHook(func() {
		hookCallCount++
	})

	identity, _ := svc.CreateIdentity(ctx, CreateIdentityInput{Name: "hook-test-user"})
	// CreateIdentity should call the hook once.
	if hookCallCount != 1 {
		t.Errorf("PostMutationHook call count after CreateIdentity = %d, want 1", hookCallCount)
	}
	...
	if hookCallCount != 1 {
		t.Errorf("PostMutationHook call count after GenerateKey = %d, want 1", hookCallCount)
	}
}

// TestIdentityService_PostMutationHook_NotCalledOnError verifies that
// PostMutationHook is NOT called when a mutation fails (e.g. identity not found).
func TestIdentityService_PostMutationHook_NotCalledOnError(t *testing.T) {
	svc, _, _ := testIdentityEnv(t)
	ctx := context.Background()
	...
	if err == nil {
		t.Fatal("GenerateKey() with nonexistent identity should return error")
	}
	if hookCallCount != 0 {
		t.Errorf("PostMutationHook should NOT be called on error, but was called %d time(s)", hookCallCount)
	}
}
