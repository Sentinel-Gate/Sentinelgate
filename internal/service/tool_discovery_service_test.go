...
func (m *discoveryMockClient) handleRequests() {
	...
	for scanner.Scan() {
		...
		// Parse request to get ID and method.
		var req struct {
		...
		// Skip notifications (no ID = no response expected per JSON-RPC spec).
		if req.ID == "" {
			continue
		}
		...
		case "initialize":
			resp = fmt.Sprintf(
				`{"jsonrpc":"2.0","id":%q,"result":{"protocolVersion":"2025-06-18","capabilities":{"tools":{"listChanged":true}},"serverInfo":{"name":"mock","version":"1.0.0"}}}`,
				req.ID,
			)
		default:
			// tools/list and other methods: return tools.
...
func TestToolDiscoveryService_ConflictDetection(t *testing.T) {
	...
	// With namespacing, ALL tools are stored (no skipping).
	// shared_tool now exists under namespaced names from both upstreams.
	_, ok := cache.GetTool("server1/shared_tool")
	if !ok {
		t.Fatal("server1/shared_tool should be in cache (upstream-1 tool, namespaced)")
	}
	_, ok = cache.GetTool("server2/shared_tool")
	if !ok {
		t.Fatal("server2/shared_tool should be in cache (upstream-2 tool, namespaced)")
	}

	_, ok = cache.GetTool("unique_tool")
	if !ok {
		t.Error("unique_tool should be in cache (no conflict, bare name)")
	}

	// count should reflect all tools (both namespaced shared + unique).
	if count != 2 {
		t.Errorf("count = %d, want 2 (both shared_tool and unique_tool discovered)", count)
	}
}
...
func TestToolDiscoveryService_PeriodicRetry(t *testing.T) {
	...
	svc.SetRetryInterval(50 * time.Millisecond)
	defer svc.Stop()
