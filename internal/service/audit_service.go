...
func (s *AuditService) Record(record audit.AuditRecord) {
	...
	// Recover from send-on-closed-channel panic (TOCTOU between stopped.Load and channel send)
	defer func() {
		if r := recover(); r != nil {
			s.recordDrop(record)
		}
...
// Pending records are flushed before returning. Safe to call multiple times.
	...
	for {
		...
		case record, ok := <-s.auditChan:
			if !ok {
				...
				// Budget (4s) is shorter than the lifecycle hook timeout (5s) so the
				// flush completes before the hook proceeds to close the store.
				if len(batch) > 0 {
					flushCtx, flushCancel := context.WithTimeout(context.Background(), 4*time.Second)
		...
		case <-ticker.C:
			// M2: Check context before flushing to avoid blocking if Stop()
			// is never called but the context has been cancelled.
			select {
			case <-ctx.Done():
				goto ctxShutdown
			default:
...
ctxShutdown:
	// Mark as stopped so new Record() calls are rejected immediately
	s.stopped.Store(true)
	// Context cancelled - drain remaining records without blocking
	for {
		select {
		case record, ok := <-s.auditChan:
			if !ok {
