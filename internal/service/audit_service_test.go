...
func TestAuditService_ChannelDepthWarning(t *testing.T) {
	...
	// NOTE: Intentionally accessing unexported field svc.auditChan for precise
	// test control — this is an internal (same-package) test.
	for i := 0; i < 9; i++ {
		select {
	...
	// NOTE: Intentionally accessing unexported field for test cleanup.
	close(svc.auditChan)
