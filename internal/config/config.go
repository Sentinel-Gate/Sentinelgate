...
type OSSConfig struct {
	...
	// Evidence configures cryptographic audit evidence (Upgrade 1).
	Evidence EvidenceConfig `yaml:"evidence" mapstructure:"evidence"`

	// Webhook configures event webhook notifications.
	Webhook WebhookConfig `yaml:"webhook" mapstructure:"webhook"`

	rateLimitEnabledExplicit bool
	evidenceEnabledExplicit  bool
}

// WebhookConfig configures a single HTTP webhook for event notifications.
type WebhookConfig struct {
	// URL is the HTTP endpoint to POST events to.
	URL string `yaml:"url" mapstructure:"url"`
	// Secret is an optional HMAC-SHA256 secret for signing payloads.
	Secret string `yaml:"secret" mapstructure:"secret"`
	// Events filters which event types trigger the webhook (empty = all).
...
// EvidenceConfig configures cryptographic evidence for audit records.
// When enabled, every audit entry is signed with ECDSA P-256 and hash-chained
// to produce tamper-proof evidence of all actions.
type EvidenceConfig struct {
	// Enabled turns cryptographic evidence on or off. Default: true.
	Enabled bool `yaml:"enabled" mapstructure:"enabled"`

	// KeyPath is the path to the ECDSA P-256 private key PEM file.
	// If the file doesn't exist, a new key is generated automatically.
	// Default: "evidence-key.pem" in the working directory.
	KeyPath string `yaml:"key_path" mapstructure:"key_path"`

	// OutputPath is the path to the evidence JSONL file (append-only).
	// Default: "evidence.jsonl" in the working directory.
	OutputPath string `yaml:"output_path" mapstructure:"output_path"`

	// SignerID is a human-readable identifier for this SentinelGate instance.
	// Used in evidence records to identify which instance signed the record.
	// Default: hostname.
	SignerID string `yaml:"signer_id" mapstructure:"signer_id"`
}
...
type RateLimitConfig struct {
	...
	// IPBurst is the maximum burst size for per-IP rate limiting.
	// Defaults to IPRate if not specified.
	IPBurst int `yaml:"ip_burst" mapstructure:"ip_burst" validate:"omitempty,min=1"`

	// UserBurst is the maximum burst size for per-user rate limiting.
	// Defaults to UserRate if not specified.
	UserBurst int `yaml:"user_burst" mapstructure:"user_burst" validate:"omitempty,min=1"`
...
type AuditFileConfig struct {
	...
	// Defaults to 1000.
	CacheSize int `yaml:"cache_size" mapstructure:"cache_size" validate:"omitempty,min=1"` // L-69
}
...
func (c *OSSConfig) SetDefaults() {
	...
	if !c.rateLimitEnabledExplicit {
		c.RateLimit.Enabled = true
	}

	// Evidence defaults — enabled by default for compliance
	if !c.evidenceEnabledExplicit {
		c.Evidence.Enabled = true
	}
	if c.Evidence.KeyPath == "" {
		c.Evidence.KeyPath = "evidence-key.pem"
	}
	if c.Evidence.OutputPath == "" {
		c.Evidence.OutputPath = "evidence.jsonl"
	}
	...
	if c.RateLimit.IPBurst == 0 {
		c.RateLimit.IPBurst = c.RateLimit.IPRate
	}
	if c.RateLimit.UserBurst == 0 {
		c.RateLimit.UserBurst = c.RateLimit.UserRate
	}
