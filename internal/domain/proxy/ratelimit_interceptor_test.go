...
func TestRateLimitInterceptor_IPAllowed(t *testing.T) {
	logger := slog.Default()
...
func TestRateLimitInterceptor_IPBlocked(t *testing.T) {
	logger := slog.Default()
...
func TestRateLimitInterceptor_UserAllowed(t *testing.T) {
	logger := slog.Default()
	...
	// Verify both IP and user rate limits were checked
	if len(checkedKeys) != 2 {
		t.Errorf("expected 2 rate limit checks, got %d", len(checkedKeys))
	}
	if checkedKeys[0] != "ratelimit:ip:192.168.1.1" {
		t.Errorf("expected IP key, got %s", checkedKeys[0])
	}
	if checkedKeys[1] != "ratelimit:user:user-456" {
		t.Errorf("expected user key, got %s", checkedKeys[1])
	}
}

func TestRateLimitInterceptor_UserBlocked(t *testing.T) {
	logger := slog.Default()
...
func TestRateLimitInterceptor_NoIPInContext(t *testing.T) {
	logger := slog.Default()
...
func TestRateLimitInterceptor_ServerToClientPassthrough(t *testing.T) {
	logger := slog.Default()
	limiter := &mockRateLimiter{
		allowFunc: func(ctx context.Context, key string, config ratelimit.RateLimitConfig) (ratelimit.RateLimitResult, error) {
