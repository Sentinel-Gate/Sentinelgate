...
type cacheEntry struct {
	...
	apiKeyHash string // SHA-256 hash of the API key used to create this session (empty if no key)
}
...
// sessionIDSlotKey is the context key for the session ID write-back slot.
type sessionIDSlotKey struct{}

// SessionIDSlotKey allows the HTTP handler to receive the domain-layer session ID
// generated by AuthInterceptor. The handler puts a *string in context; after the
// interceptor chain runs, it reads the value to set the Mcp-Session-Id header.
// This ensures the client-visible session ID matches the audit record session_id.
var SessionIDSlotKey = sessionIDSlotKey{}
...
func SafeErrorMessage(err error) string {
	...
	case errors.Is(err, ErrContentBlocked):
		return "Blocked by content scanning: sensitive data detected"
	case errors.Is(err, ErrResponseBlocked):
		return "Response blocked: potential prompt injection detected"
	case errors.Is(err, ErrOutboundBlocked):
		return "Blocked by outbound security rules"
	default:
...
func (a *AuthInterceptor) Intercept(ctx context.Context, msg *mcp.Message) (*mcp.Message, error) {
	...
	if hasCachedSession {
		...
		// SECURITY: If the current request has an API key, verify it matches
		// the key used to create the cached session. This prevents session
		// fixation attacks on shared connIDs (e.g., stdio transport where
		// all clients share connID="default").
		if apiKey != "" && entry.apiKeyHash != "" && apiKeyHashForCache(apiKey) != entry.apiKeyHash {
			a.sessionMu.Lock()
			delete(a.sessionCache, connID)
			a.sessionMu.Unlock()
	...
	if hasCachedSession {
		...
		if err == nil && !sess.IsExpired() {
			...
			msg.Session = sess
			// Write session ID to the HTTP handler's slot so Mcp-Session-Id matches audit records
			if slot, ok := ctx.Value(SessionIDSlotKey).(*string); ok {
				*slot = sess.ID
			}
	...
	a.sessionCache[connID] = &cacheEntry{
		...
		lastAccess: time.Now(),
		apiKeyHash: apiKeyHashForCache(apiKey),
	}
	a.sessionMu.Unlock()
...
func (a *AuthInterceptor) InvalidateByIdentity(identityID string) {
	a.sessionMu.Lock()
...
// apiKeyHashForCache returns the SHA-256 hash of the API key for cache comparison.
// Returns empty string for empty keys (e.g., cached sessions without API key).
func apiKeyHashForCache(key string) string {
	if key == "" {
		return ""
	}
	return auth.HashKey(key) //nolint:staticcheck // SA1019: intentional use for backward-compatible cache lookup
}
