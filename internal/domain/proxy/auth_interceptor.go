...
type AuthInterceptor struct {
	...
	// When the cache reaches this limit, the oldest entry (by lastAccess) is evicted.
...
// defaultMaxCacheSize is the default maximum number of entries in the session cache.
const defaultMaxCacheSize = 10000

// NewAuthInterceptor creates a new AuthInterceptor with default cleanup settings.
// Default cleanupInterval: 5 minutes, default cacheMaxAge: 30 minutes, default maxCacheSize: 10000.
...
// maxCacheSize=0 uses the default (10000).
...
func (a *AuthInterceptor) Intercept(ctx context.Context, msg *mcp.Message) (*mcp.Message, error) {
	...
	if hasCachedSession && msg.Method() == "initialize" {
		a.sessionMu.Lock()
...
func (a *AuthInterceptor) evictOldestLocked() {
