...
func (m *mockAuthStore) GetAPIKeyByPrefix(ctx context.Context, prefix string) (*auth.APIKey, error) {
...
func TestAuthInterceptor_ValidAPIKey(t *testing.T) {
	...
	authStore.AddKey(&auth.APIKey{
		Key:        auth.HashKey("valid-api-key"), //nolint:staticcheck // SA1019: backward-compatible key lookup
		IdentityID: "user-1",
		CreatedAt:  time.Now(),
		Revoked:    false,
	})
...
func TestSessionCacheMaxSize(t *testing.T) {
	// Setup: identity + key for authentication
	authStore := newMockAuthStore()
	authStore.AddIdentity(&auth.Identity{
		ID:    "user-1",
		Name:  "Test User",
	...
	authStore.AddKey(&auth.APIKey{
		Key:        auth.HashKey("cap-test-key"), //nolint:staticcheck // SA1019: backward-compatible key lookup
		IdentityID: "user-1",
		Revoked:    false,
	})
	...
	interceptor.maxCacheSize = 3

	// Authenticate 5 different connections — each Intercept call adds to the cache.
	// Cache must never exceed maxCacheSize (3).
	for i := 0; i < 5; i++ {
		connID := "cap-conn-" + string(rune('A'+i))
		ctx := context.WithValue(context.Background(), ConnectionIDKey, connID)
		msg := createTestMessage("cap-test-key")
		_, err := interceptor.Intercept(ctx, msg)
		if err != nil {
			t.Fatalf("Intercept() conn %s error = %v", connID, err)
		}

		if interceptor.CacheSize() > interceptor.maxCacheSize {
			t.Errorf("cache size %d exceeds maxCacheSize %d after conn %s",
				interceptor.CacheSize(), interceptor.maxCacheSize, connID)
		}
	}

	// After 5 connections with cap=3, cache must have exactly 3 entries.
	if interceptor.CacheSize() != interceptor.maxCacheSize {
		t.Errorf("expected cache size %d, got %d", interceptor.maxCacheSize, interceptor.CacheSize())
	}
}
