...
func (r *UpstreamRouter) handleToolsCall(ctx context.Context, msg *mcp.Message) (*mcp.Message, error) {
	...
	return r.forwardToUpstream(tool.UpstreamID, msg, writer, reader)
}
...
func (r *UpstreamRouter) handleForward(ctx context.Context, msg *mcp.Message) (*mcp.Message, error) {
	...
	if len(allTools) > 0 {
		...
		if err == nil {
			return r.forwardToUpstream(upstreamID, msg, writer, reader)
		}
	...
	return r.forwardToUpstream("primary", msg, writer, reader)
}
...
func (r *UpstreamRouter) forwardToUpstream(upstreamID string, msg *mcp.Message, writer io.WriteCloser, reader io.ReadCloser) (*mcp.Message, error) {
	// Serialize access to this upstream's stdin/stdout pipes.
	...
	// The upstream may use its own internal ID numbering, but the client
	// expects the response ID to match the request ID it originally sent.
	clientID := msg.RawID()
	if clientID != nil {
