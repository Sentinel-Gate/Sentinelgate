...
// Deprecated: use action.ActionAuditInterceptor for new code.
...
func (a *AuditInterceptor) Intercept(ctx context.Context, msg *mcp.Message) (*mcp.Message, error) {
	...
	// Create transform result holder in context for downstream interceptors.
	...
	// Invoke recording callback in a goroutine to ensure zero latency impact.
	// The callback writes to the session JSONL file; errors are handled internally.
	a.cbMu.RLock()
...
// each tool call is audited. The callback receives a copy of the AuditRecord and
// is called in a new goroutine to prevent any recording latency from affecting the
// proxy path. Pass nil to remove the callback.
func (a *AuditInterceptor) SetRecordingCallback(cb func(audit.AuditRecord)) {
	a.cbMu.Lock()
