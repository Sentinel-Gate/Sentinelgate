...
// regexCacheEntry pairs a compiled regex with its LRU list element.
...
// maxRegexCacheSize is the maximum number of compiled regex entries kept in the cache.
// When exceeded, the entire cache is cleared to prevent unbounded memory growth.
const maxRegexCacheSize = 500

// getOrCompileRegex returns a compiled regex from the cache, or compiles and caches it.
// L-4: Re-checks cache inside write lock to avoid spurious eviction under concurrency.
// L-32: Uses LRU eviction instead of full-flush when cache exceeds maxRegexCacheSize.
func (e *TransformExecutor) getOrCompileRegex(pattern string) (*regexp.Regexp, error) {
	e.cacheMu.RLock()
	if elem, ok := e.regexCache[pattern]; ok {
		e.cacheMu.RUnlock()
	...
	e.cacheMu.Lock()
	// L-4: Re-check cache inside lock to avoid duplicate insert and spurious eviction.
	if elem, ok := e.regexCache[pattern]; ok {
		e.lruList.MoveToFront(elem)
		e.cacheMu.Unlock()
		return elem.Value.(*regexCacheEntry).re, nil
	}
	// L-32: Evict least-recently-used entry instead of flushing entire cache.
	for e.lruList.Len() >= maxRegexCacheSize {
		tail := e.lruList.Back()
		if tail == nil {
			break
		}
		evicted := tail.Value.(*regexCacheEntry)
		delete(e.regexCache, evicted.pattern)
		e.lruList.Remove(tail)
	}
	entry := &regexCacheEntry{pattern: pattern, re: re}
	elem := e.lruList.PushFront(entry)
	e.regexCache[pattern] = elem
	e.cacheMu.Unlock()
	return re, nil
}
