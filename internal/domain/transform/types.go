...
// maxPatternLength is the maximum allowed length for a regex pattern string.
// L-10: Prevents ReDoS via excessively long patterns supplied through the admin API.
...
func (r *TransformRule) Validate() error {
	...
	case TransformRedact:
		...
		for i, p := range r.Config.Patterns {
			if len(p) > maxPatternLength {
				return fmt.Errorf("redact pattern[%d] exceeds max length %d", i, maxPatternLength)
			}
	...
	case TransformMask:
		...
		for i, p := range r.Config.MaskPatterns {
			if len(p) > maxPatternLength {
				return fmt.Errorf("mask pattern[%d] exceeds max length %d", i, maxPatternLength)
			}
			if _, err := regexp.Compile(p); err != nil {
...
// SortByPriority returns a copy of the rules sorted by Priority descending
// (higher priority number = runs first). Matches policy rule behavior.
