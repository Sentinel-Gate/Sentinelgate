...
func TestTransformRule_Validate_MissingToolMatch(t *testing.T) {
	...
	// L-3: Use strings.Contains for resilient error message checking.
	if !strings.Contains(err.Error(), "tool_match is required") {
		t.Errorf("unexpected error: %v", err)
	}
}
...
func TestSortByPriority(t *testing.T) {
	...
	// Descending: higher priority number = runs first
	expected := []string{"third", "second", "first", "also-first"}
