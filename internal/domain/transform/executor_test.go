...
func TestExecutor_MultipleRules_PriorityOrder(t *testing.T) {
	...
	// Redact (priority 10) runs first
	if results[0].RuleName != "redact-key" {
		t.Errorf("expected redact first, got: %s", results[0].RuleName)
	}
	// Inject (priority 20) runs second
	if results[1].RuleName != "inject-footer" {
		t.Errorf("expected inject second, got: %s", results[1].RuleName)
	}
