...
func TestResponseScanInterceptor_TypedNilMCPMessage_NoPanic(t *testing.T) {
	// Regression test: when OriginalMessage holds a typed nil (*mcp.Message)(nil),
	// the type assertion succeeds (ok=true) but mcpMsg is nil. Accessing mcpMsg.Direction
	// would panic. This simulates what happens when the upstream router returns nil
	// for notifications and the LegacyAdapter stores the typed nil in OriginalMessage.
	scanner := NewResponseScanner()

	typedNilAction := &CanonicalAction{
		Type:            ActionToolCall,
		Name:            "test",
		Protocol:        "mcp",
		OriginalMessage: (*mcp.Message)(nil), // typed nil — the root cause of the crash
	}

	next := scanMockNext(typedNilAction, nil)
	interceptor := NewResponseScanInterceptor(scanner, next, ScanModeEnforce, true, testLogger())

	// This must NOT panic.
	result, err := interceptor.Intercept(context.Background(), typedNilAction)
	if err != nil {
		t.Fatalf("typed nil mcpMsg should skip scanning, got error: %v", err)
	}
	if result == nil {
		t.Fatal("expected non-nil result for typed nil mcpMsg")
	}
}
