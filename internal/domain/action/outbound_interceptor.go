...
// GetRules returns a copy of the current outbound rules.
// Used by the CONNECT filter to evaluate destinations without going through
// the full interceptor chain.
func (o *OutboundInterceptor) GetRules() []OutboundRule {
	rules := *o.rules.Load()
	cp := make([]OutboundRule, len(rules))
	copy(cp, rules)
	return cp
}
