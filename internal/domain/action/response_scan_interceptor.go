...
// ErrResponseBlocked is an alias for proxy.ErrResponseBlocked for backward compatibility.
var ErrResponseBlocked = proxy.ErrResponseBlocked
...
func (r *ResponseScanInterceptor) Intercept(ctx context.Context, a *CanonicalAction) (*CanonicalAction, error) {
	...
	if !ok || mcpMsg == nil {
		// Non-MCP message or typed nil (e.g., notification with no response), skip scanning.
		return result, nil
	}
	...
	// Emit IPI detection event for notifications.
	r.mu.RLock()
	bus := r.eventBus
	r.mu.RUnlock()
	if bus != nil {
		...
		bus.Publish(ctx, event.Event{
			Type:     "content.ipi_detected",
			Source:   "response-scanner",
			Severity: severity,
			Payload: map[string]interface{}{
				...
				"patterns":      strings.Join(patternNames, ", "),
				"mode":          string(currentMode),
			},
			RequiresAction: currentMode == ScanModeEnforce,
		})
	}
...
func (r *ResponseScanInterceptor) Mode() ScanMode {
	m, ok := r.mode.Load().(ScanMode)
	if !ok {
		return ScanModeMonitor
	}
	return m
}
...
// SetEventBus sets the event bus for emitting IPI detection events.
func (r *ResponseScanInterceptor) SetEventBus(bus event.Bus) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.eventBus = bus
}
