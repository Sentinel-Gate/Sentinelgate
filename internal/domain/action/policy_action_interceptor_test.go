...
func TestPolicyActionInterceptor_NonToolCallPassthrough(t *testing.T) {
	...
	// Protocol action should bypass policy evaluation
	...
	if engineCalled {
		t.Error("policy engine should NOT be called for protocol actions")
	}
	if !next.called {
...
func TestPolicyActionInterceptor_SamplingEvaluated(t *testing.T) {
	engineCalled := false
	engine := &mockPolicyEngine{
		evaluateFn: func(ctx context.Context, evalCtx policy.EvaluationContext) (policy.Decision, error) {
			engineCalled = true
	...
	// Sampling action should now be evaluated by policy engine
	action := &CanonicalAction{
		Type:     ActionSampling,
	...
	if !engineCalled {
		t.Error("policy engine should be called for sampling actions")
	}
	if !next.called {
		t.Error("next interceptor should be called when allowed")
	}
}
...
func TestPolicyActionInterceptor_SessionHistory(t *testing.T) {
	...
	// Verify session action history was populated (2 from history + 1 current action)
	if len(capturedCtx.SessionActionHistory) != 3 {
		t.Fatalf("SessionActionHistory len = %d, want 3 (2 history + 1 current)", len(capturedCtx.SessionActionHistory))
	}
	...
	// Last entry is the current action (read_file from newTestToolCallAction)
	if capturedCtx.SessionActionHistory[2].ToolName != "read_file" {
		t.Errorf("SessionActionHistory[2].ToolName = %q, want %q (current action)", capturedCtx.SessionActionHistory[2].ToolName, "read_file")
	}

	// Verify action set includes both history and current action
...
// TestPolicyActionInterceptor_SessionSequenceCurrentAction verifies that the
// current action is included in SessionActionHistory so that session_sequence
// can detect sequences like "read_file happened before send_email (current)".
// This is the bug reported by the playground team: session_sequence never fired
// because the current action wasn't in the history during policy evaluation.
func TestPolicyActionInterceptor_SessionSequenceCurrentAction(t *testing.T) {
	now := time.Now()
	...
	// Simulate: read_file was called previously in this session
	provider := &mockSessionUsageProvider{
		usage: SessionUsageData{
			TotalCalls: 1,
			StartedAt:  now.Add(-1 * time.Minute),
			ActionHistory: []SessionActionRecord{
				{ToolName: "read_file", CallType: "read", Timestamp: now.Add(-30 * time.Second)},
			},
			ActionSet: map[string]bool{"read_file": true},
	...
	// History should be [read_file, send_email] (1 from history + 1 current)
	if len(capturedCtx.SessionActionHistory) != 2 {
		t.Fatalf("SessionActionHistory len = %d, want 2", len(capturedCtx.SessionActionHistory))
	}
	if capturedCtx.SessionActionHistory[0].ToolName != "read_file" {
		t.Errorf("[0].ToolName = %q, want read_file", capturedCtx.SessionActionHistory[0].ToolName)
	}
	if capturedCtx.SessionActionHistory[1].ToolName != "send_email" {
		t.Errorf("[1].ToolName = %q, want send_email (current action)", capturedCtx.SessionActionHistory[1].ToolName)
	}

	// Action set should include both
	if !capturedCtx.SessionActionSet["read_file"] {
		t.Error("SessionActionSet should contain read_file")
	}
	if !capturedCtx.SessionActionSet["send_email"] {
		t.Error("SessionActionSet should contain send_email (current action)")
	}

	// Arg key set should include current action's argument keys
	if !capturedCtx.SessionArgKeySet["to"] {
		t.Error("SessionArgKeySet should contain 'to' (current action arg key)")
	}
	if !capturedCtx.SessionArgKeySet["subject"] {
		t.Error("SessionArgKeySet should contain 'subject' (current action arg key)")
	}
}

// TestPolicyActionInterceptor_SessionHistory_EmptyHistory verifies that when
// there is no prior history (first action in session), the current action
// still appears in SessionActionHistory.
func TestPolicyActionInterceptor_SessionHistory_EmptyHistory(t *testing.T) {
	now := time.Now()
	...
	// Empty session — no prior actions
	provider := &mockSessionUsageProvider{
		usage: SessionUsageData{
			TotalCalls: 0,
	...
	act := newTestToolCallAction() // read_file with arg "path"
	_, err := interceptor.Intercept(context.Background(), act)
	if err != nil {
		t.Fatalf("Intercept() error = %v", err)
	}

	// History should contain only the current action
	if len(capturedCtx.SessionActionHistory) != 1 {
		t.Fatalf("SessionActionHistory len = %d, want 1 (current action only)", len(capturedCtx.SessionActionHistory))
	}
	if capturedCtx.SessionActionHistory[0].ToolName != "read_file" {
		t.Errorf("[0].ToolName = %q, want read_file", capturedCtx.SessionActionHistory[0].ToolName)
	}

	// Action set and arg key set should include the current action
	if !capturedCtx.SessionActionSet["read_file"] {
		t.Error("SessionActionSet should contain read_file (current action)")
	}
	if !capturedCtx.SessionArgKeySet["path"] {
		t.Error("SessionArgKeySet should contain 'path' (current action arg key)")
	}
}
