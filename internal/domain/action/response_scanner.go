...
func NewResponseScanner() *ResponseScanner {
	rawPatterns := []struct {
		...
		// Core prompt injection patterns.
		{
		...
		{
			name:     "model_delimiter_escape",
			category: "context_escape",
			pattern:  `<\|(?:im_start|im_end|endoftext|sep|pad)\|>`,
		},
		{
			name:     "inst_delimiter",
			category: "context_escape",
		...
		},
		{
			...
			pattern:  `(?i)(?:IMPORTANT|CRITICAL|URGENT):\s*(?:ignore|override|disregard|forget|replace)`,
		},
		{
			name:     "context_switch",
			category: "instruction_override",
			pattern:  `(?i)(?:end\s+of\s+(?:context|conversation|instructions)|begin\s+new\s+(?:session|context|instructions))`,
		},
		{
			name:     "tool_poisoning_directive",
			category: "prompt_injection",
			pattern:  `(?i)(?:also|additionally|before\s+responding|after\s+this)[,;]?\s+(?:read|execute|run|call|send|include|fetch|download|upload)\s+`,
		},
	}
