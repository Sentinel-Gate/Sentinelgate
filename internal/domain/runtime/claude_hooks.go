...
func writeClaudeHook(settingsPath string, cfg ClaudeHookConfig) error {
	...
	// Use forward slashes in the executable path so the command works when
	// Claude Code executes it via sh -c (Git Bash) on Windows. Backslashes
	// are interpreted as escape characters by POSIX shells, corrupting the path.
	exePath := filepath.ToSlash(cfg.SentinelGateExe)
