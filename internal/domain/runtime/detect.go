...
// IsClaudeCode checks if the given command is Claude Code.
// First tries IsBunBinary (works on macOS where Claude is a Bun-compiled binary).
// If that fails, resolves the command via LookPath and reads the wrapper file
// to check for the @anthropic-ai/claude-code package reference — this handles
// Windows where Claude Code is installed via npm and the command resolves to
// a .cmd/.ps1 wrapper script.
func IsClaudeCode(command string) bool {
	if IsBunBinary(command) {
		return true
	}

	// Resolve the command to its full path (e.g. claude -> claude.cmd on Windows).
	path, err := exec.LookPath(command)
	if err != nil {
		return false
	}
	...
	content := string(data)
	// npm sh wrapper uses forward slashes: @anthropic-ai/claude-code
	// npm .cmd wrapper uses backslashes: @anthropic-ai\claude-code
	return strings.Contains(content, "@anthropic-ai/claude-code") ||
		strings.Contains(content, `@anthropic-ai\claude-code`)
}
