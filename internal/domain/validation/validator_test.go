...
// FuzzMessageValidator tests Validate with arbitrary JSON input.
// Must not panic on any input (A7e: fuzz testing for critical parsers).
func FuzzMessageValidator(f *testing.F) {
	// Seed corpus
	f.Add([]byte(`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"test"}}`))
	f.Add([]byte(`{"jsonrpc":"2.0","id":1,"result":{}}`))
	f.Add([]byte(`{}`))
	f.Add([]byte(`null`))
	f.Add([]byte(``))
	f.Add([]byte(`{"jsonrpc":"2.0","id":1,"method":"","params":null}`))
	f.Add([]byte(`{"jsonrpc":"2.0","method":"tools/call","params":{"name":"x","arguments":{"<script>":"evil"}}}`))

	f.Fuzz(func(t *testing.T, data []byte) {
		v := NewMessageValidator()

		// Try to decode as JSON-RPC, then validate.
		// Must not panic on any input.
		decoded, err := mcp.DecodeMessage(data)
		if err != nil {
			// Can't decode → create message with nil Decoded
			msg := &mcp.Message{Raw: data}
			_ = v.Validate(msg)
			return
		}
		...
		// Parse params if it's a request
		if req, ok := decoded.(*jsonrpc.Request); ok && req.Params != nil {
