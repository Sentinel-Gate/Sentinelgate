...
type mockAuthStore struct {
	...
	prefixIndex map[string]string // prefix -> keyHash
}
...
func (m *mockAuthStore) GetAPIKeyByPrefix(ctx context.Context, prefix string) (*APIKey, error) {
	keyHash, ok := m.prefixIndex[prefix]
	if !ok {
		return nil, errKeyNotFound
	}
	key, ok := m.keys[keyHash]
...
// addKeyWithPrefix adds a key and registers it in the prefix index if Prefix is set.
func (m *mockAuthStore) addKeyWithPrefix(key *APIKey) {
	m.keys[key.Key] = key
	if key.Prefix != "" {
		m.prefixIndex[key.Prefix] = key.Key
	}
}
...
func TestValidate_Argon2idFastPath(t *testing.T) {
	// This test verifies that Argon2id validation uses the prefix index
	// (O(1)) rather than iterating all keys (O(n)).
	rawKey := "sg_ui_abcdef1234567890"
	argon2Hash, err := HashKeyArgon2id(rawKey)
	if err != nil {
		t.Fatalf("HashKeyArgon2id() error = %v", err)
	}
	...
	store := newMockAuthStore()
	store.addKeyWithPrefix(&APIKey{
		Key:        argon2Hash,
		Prefix:     rawKey[:8],
		IdentityID: "user-prefix",
		Revoked:    false,
	})
	store.identities["user-prefix"] = &Identity{
		ID:    "user-prefix",
		Name:  "Prefix User",
		Roles: []Role{RoleUser},
	}

	// Also add a second key without a prefix (to ensure iteration fallback still works)
	otherKey := "sg_other_key_12345678"
	otherHash, err := HashKeyArgon2id(otherKey)
	if err != nil {
		t.Fatalf("HashKeyArgon2id() second error = %v", err)
	}
	// No prefix — will use iteration fallback
	store.keys[otherHash] = &APIKey{
		Key:        otherHash,
		Prefix:     "", // no prefix stored
		IdentityID: "user-other",
		Revoked:    false,
	}
	store.identities["user-other"] = &Identity{
		ID:    "user-other",
		Name:  "Other User",
		Roles: []Role{RoleUser},
	}
	...
	// Test 1: prefix-indexed key resolves correctly
	identity, err := svc.Validate(context.Background(), rawKey)
	if err != nil {
		t.Fatalf("Validate() with prefix-indexed key error = %v", err)
	}
	if identity.ID != "user-prefix" {
		t.Errorf("Validate() identity.ID = %q, want %q", identity.ID, "user-prefix")
	}

	// Test 2: key without prefix falls back to iteration and resolves correctly
	identity2, err := svc.Validate(context.Background(), otherKey)
	if err != nil {
		t.Fatalf("Validate() with no-prefix key error = %v", err)
	}
	if identity2.ID != "user-other" {
		t.Errorf("Validate() identity.ID = %q, want %q", identity2.ID, "user-other")
	}

	// Test 3: wrong key returns ErrInvalidKey
	_, err = svc.Validate(context.Background(), "completely-wrong-key-not-matching")
	if err != ErrInvalidKey {
		t.Errorf("Validate() with wrong key = %v, want ErrInvalidKey", err)
	}
}
