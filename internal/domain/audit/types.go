...
const (
	...
	DecisionAllow = "allow"
	// DecisionDeny indicates the tool call was blocked by policy.
	DecisionDeny = "deny"
	// DecisionBlocked indicates the tool call was blocked by quota enforcement.
	DecisionBlocked = "blocked"
	// DecisionWarn indicates the tool call was allowed but a quota warning was emitted.
	DecisionWarn = "warn"
)
...
type AuditRecord struct {
	...
	// Roles assigned to the identity at decision time.
	...
	// TransformResults lists the transforms applied to this tool call's response.
	// Empty if no transforms were applied.
	TransformResults []TransformApplied `json:"transform_results,omitempty"`

	// ResponseBody holds the tool response text from the upstream server.
	// Populated when the tool call is allowed and a response is received.
	ResponseBody string `json:"response_body,omitempty"`

	// Source indicates the origin of the audit record (M-19).
	// Empty for real traffic; "admin_evaluate" for policy evaluate endpoint simulations.
