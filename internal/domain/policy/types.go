...
type Rule struct {
	...
	Name string
	// Priority determines rule evaluation order (higher number = higher priority).
	Priority int
	...
	// Empty for manually created rules.
	Source string
}
...
type Decision struct {
	...
	RuleName string
	// Priority is the priority of the matched rule (used for simulation comparisons).
	Priority int
...
type Policy struct {
	...
	Description string
	// Priority determines policy evaluation order (higher number = higher priority).
	Priority int
