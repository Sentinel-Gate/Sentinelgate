...
// TestStdioTransport_SendNotification verifies that SendNotification writes
// a valid JSON-RPC notification to stdout with a trailing newline.
func TestStdioTransport_SendNotification(t *testing.T) {
	...
	// Must be valid JSON-RPC notification
	var notification struct {
		JSONRPC string `json:"jsonrpc"`
		Method  string `json:"method"`
	}
	if err := json.Unmarshal([]byte(strings.TrimSpace(output)), &notification); err != nil {
		t.Fatalf("invalid JSON: %v, got %q", err, output)
	}
...
// TestStdioToolChangeNotifier verifies that the notifier correctly delegates
// to StdioTransport.SendNotification.
func TestStdioToolChangeNotifier(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	mockClient := &mockMCPClient{}
	proxyService := service.NewProxyService(mockClient, proxy.NewPassthroughInterceptor(), logger)
	transport := NewStdioTransport(proxyService)

	notifier := NewStdioToolChangeNotifier(transport)
