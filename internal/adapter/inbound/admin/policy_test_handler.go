...
type PolicyTestRequest struct {
	...
	// SessionCumulativeCost is a simulated cumulative cost for the current session.
	SessionCumulativeCost float64 `json:"session_cumulative_cost,omitempty"`
	// SessionContext is an optional list of prior actions to simulate a session history.
	// Each entry has tool_name (required), call_type (optional, defaults to "other"),
	// seconds_ago (optional, defaults to 0), and arg_keys (optional).
	SessionContext []SessionContextEntry `json:"session_context,omitempty"`
}

// SessionContextEntry represents a single prior action in the simulated session history.
...
func (h *AdminAPIHandler) handleTestPolicy(w http.ResponseWriter, r *http.Request) {
	...
	// Resolve identity_name and roles from identity_id if not provided.
	if req.IdentityID != "" && h.identityService != nil {
		if identity, err := h.identityService.GetIdentity(r.Context(), req.IdentityID); err == nil {
			if req.IdentityName == "" {
				req.IdentityName = identity.Name
			}
			if len(req.Roles) == 0 {
	...
	// SkipCache ensures playground always shows fresh results,
	// especially important for session-aware policy testing.
	evalCtx := policy.EvaluationContext{
		...
		ActionName:    req.ToolName, // alias for CEL action_name variable
		...
		SessionCumulativeCost: req.SessionCumulativeCost,
		SkipCache:             true,
	}

	// Populate session context from simulated prior actions.
	if len(req.SessionContext) > 0 {
		now := time.Now()
		for _, entry := range req.SessionContext {
			callType := entry.CallType
			if callType == "" {
				callType = "other"
			}
			ts := now.Add(-time.Duration(entry.SecondsAgo) * time.Second)
			evalCtx.SessionActionHistory = append(evalCtx.SessionActionHistory, policy.SessionActionRecord{
				ToolName:  entry.ToolName,
				CallType:  callType,
				Timestamp: ts,
				ArgKeys:   entry.ArgKeys,
			})
			if evalCtx.SessionActionSet == nil {
				evalCtx.SessionActionSet = make(map[string]bool)
			}
			evalCtx.SessionActionSet[entry.ToolName] = true
			if evalCtx.SessionArgKeySet == nil {
				evalCtx.SessionArgKeySet = make(map[string]bool)
			}
			for _, k := range entry.ArgKeys {
				evalCtx.SessionArgKeySet[k] = true
			}
		}
		// Append the current tool call as the latest action in the history.
		// This allows session_sequence rules to detect patterns where the
		// current tool call is the second action (e.g., read_file -> send_email).
		evalCtx.SessionActionHistory = append(evalCtx.SessionActionHistory, policy.SessionActionRecord{
			ToolName:  req.ToolName,
			CallType:  "other",
			Timestamp: now,
		})
		evalCtx.SessionActionSet[req.ToolName] = true
		// Set session counts from the simulated history (includes current call).
		evalCtx.SessionCallCount = int64(len(req.SessionContext) + 1)
	}
