...
func TestAPIRateLimit_LocalhostBypass(t *testing.T) {
	h := NewAdminAPIHandler()
	handler := h.apiRateLimitMiddleware(1, 1*time.Minute, okHandler())
...
// TestAPIRateLimit_XFFLocalhostBypass verifies that the rate limiter correctly bypasses
// rate limiting when the XFF-resolved IP is localhost via a trusted proxy.
func TestAPIRateLimit_XFFLocalhostBypass(t *testing.T) {
	h := NewAdminAPIHandler(WithTrustedProxies([]string{"10.0.0.0/8"}))
	handler := h.apiRateLimitMiddleware(1, 1*time.Minute, okHandler())

	// Many requests from "localhost" via trusted proxy should all bypass.
	for i := 0; i < 10; i++ {
		req := httptest.NewRequest(http.MethodGet, "/admin/api/test", nil)
		req.RemoteAddr = "10.0.0.1:1234"
