...
func (h *AdminAPIHandler) handleCreateTransform(w http.ResponseWriter, r *http.Request) {
	...
	// Persist to state.json — return error if persist fails (M-13).
	if err := h.persistTransforms(r.Context()); err != nil {
		// Rollback: remove the rule we just created.
		_ = h.transformStore.Delete(r.Context(), rule.ID)
		h.logger.Error("failed to persist transforms to state", "error", err)
		h.respondError(w, http.StatusInternalServerError, "transform created in memory but failed to persist to state")
		return
	}
...
func (h *AdminAPIHandler) handleUpdateTransform(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.persistTransforms(r.Context()); err != nil {
		...
		_ = h.transformStore.Put(r.Context(), &snapshot)
		h.logger.Error("failed to persist transforms to state", "error", err)
		h.respondError(w, http.StatusInternalServerError, "transform updated in memory but failed to persist to state")
		return
	}
...
func (h *AdminAPIHandler) handleDeleteTransform(w http.ResponseWriter, r *http.Request) {
	...
	// Capture existing rule for rollback on persist failure.
	existing, err := h.transformStore.Get(r.Context(), id)
	if err != nil {
		if errors.Is(err, transform.ErrTransformNotFound) {
			h.respondError(w, http.StatusNotFound, "transform rule not found")
		...
		h.logger.Error("failed to get transform for delete", "id", id, "error", err)
	...
	// Persist to state.json — return error if persist fails (M-13).
	if err := h.persistTransforms(r.Context()); err != nil {
		// Rollback: re-add deleted rule.
		_ = h.transformStore.Put(r.Context(), existing)
		h.logger.Error("failed to persist transforms to state", "error", err)
		h.respondError(w, http.StatusInternalServerError, "transform deleted in memory but failed to persist to state")
		return
	}
...
func (h *AdminAPIHandler) handleTestTransform(w http.ResponseWriter, r *http.Request) {
	...
	for i, rr := range req.Rules {
		...
		if err := rule.Validate(); err != nil {
			h.logger.Warn("transform rule validation failed", "index", i, "error", err)
			h.respondError(w, http.StatusBadRequest, "rule["+strconv.Itoa(i)+"]: invalid configuration")
			return
		}
...
func (h *AdminAPIHandler) persistTransforms(ctx context.Context) error {
	...
	entries := make([]state.TransformRuleEntry, 0, len(rules))
	for _, r := range rules {
		var cfgMap map[string]interface{}
		if cfgBytes, err := json.Marshal(r.Config); err == nil {
			_ = json.Unmarshal(cfgBytes, &cfgMap)
		}
		entries = append(entries, state.TransformRuleEntry{
			ID:        r.ID,
			Name:      r.Name,
			Type:      string(r.Type),
