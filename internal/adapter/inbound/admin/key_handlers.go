...
// BUG-6 FIX: After revoking, invalidates all cached sessions for the key's
// identity so the agent is immediately disconnected.
func (h *AdminAPIHandler) handleRevokeKey(w http.ResponseWriter, r *http.Request) {
	...
	// BUG-6 FIX: Look up the key's identity ID before revoking, so we can
	// invalidate all cached sessions for that identity.
	var identityID string
	if keys, err := h.identityService.ListAllKeys(ctx); err == nil {
		for _, k := range keys {
			if k.ID == id {
				identityID = k.IdentityID
				break
	...
	// BUG-6 FIX: Invalidate all cached sessions for this identity so the
	// agent cannot continue operating with the revoked key.
	if identityID != "" && h.sessionCacheInvalidator != nil {
		h.sessionCacheInvalidator.InvalidateByIdentity(identityID)
	}
