...
// SetContentScanInterceptor sets the input content scan interceptor after construction.
func (h *AdminAPIHandler) SetContentScanInterceptor(i *action.ContentScanInterceptor) {
	h.contentScanInterceptor = i
}
...
func (h *AdminAPIHandler) handleUpdateContentScanning(w http.ResponseWriter, r *http.Request) {
	...
	// If mode is omitted, preserve the current mode (allows updating only the enabled flag).
	if req.Mode == "" {
		req.Mode = string(h.responseScanCtrl.Mode())
	}
	...
	if h.stateStore != nil {
		if err := h.persistContentScanningConfig(req); err != nil {
			...
			h.respondError(w, http.StatusInternalServerError, "failed to persist content scanning config")
			return
		}
	}
