...
// --- Empty Condition Default Behavior Tests ---

func TestHandlePolicies_Create_EmptyConditionDefaultsToTrue(t *testing.T) {
	h, _ := testPolicyHandlerEnv(t)

	// Send a policy with a rule that has empty condition
	body := `{"name":"Empty Cond Policy","description":"Test empty condition default","priority":10,"enabled":true,"rules":[{"name":"rule-empty-cond","priority":100,"tool_match":"*","condition":"","action":"allow"}]}`
	req := httptest.NewRequest(http.MethodPost, "/admin/api/policies", bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	...
	if created.ID == "" {
		t.Error("created policy should have an ID")
	}
	if len(created.Rules) != 1 {
		t.Fatalf("expected 1 rule, got %d", len(created.Rules))
	}

	// The empty condition should have been defaulted to "true" by toDomainPolicy.
	if created.Rules[0].Condition != "true" {
		t.Errorf("expected empty condition to be defaulted to \"true\", got %q", created.Rules[0].Condition)
	}
	if created.Rules[0].Action != "allow" {
		t.Errorf("expected action \"allow\", got %q", created.Rules[0].Action)
	}
}
