...
func (m *mockIntegrationAuditReader) Query(_ context.Context, filter audit.AuditFilter) ([]audit.AuditRecord, string, error) {
...
func TestIntegrationUpstreamLifecycle(t *testing.T) {
	...
	createReq := map[string]interface{}{
		"name":    "test-upstream",
		"type":    "http",
		"url":     "http://93.184.216.34:9999/mcp",
		"enabled": true,
	}
...
func TestIntegrationStatsAndAudit(t *testing.T) {
	...
	// L-13: go_version, os, arch are no longer exposed (runtime fingerprint redaction).
	for _, field := range []string{"version", "commit", "build_date", "uptime", "uptime_seconds"} {
