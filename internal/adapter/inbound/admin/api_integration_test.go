...
func TestIntegrationPolicyFlow(t *testing.T) {
	...
	// Step 4: Test policy - tool "other_tool" with user role -> should be denied (no matching rule -> default deny)
	testReq = map[string]interface{}{
	...
	if resp.StatusCode != http.StatusOK {
		body := readBody(t, resp)
		t.Fatalf("test policy (deny): status=%d, body=%s", resp.StatusCode, body)
	}
	decodeJSON(t, resp, &testResult)
	if testResult["decision"] != "deny" {
		t.Errorf("other_tool with user: decision=%v, want deny", testResult["decision"])
	}
