...
(function () {
  ...
  if (!SG.router.routes['access']) {
    SG.router.register('access', placeholder('Access Management', 'key', 'a future release'));
  }

  if (!SG.router.routes['audit']) {
    SG.router.register('audit', placeholder('Audit Log', 'scrollText', 'a future release'));
  }
