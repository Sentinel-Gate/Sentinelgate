...
(function () {
  ...
  function renderGroup(group, index) {
    ...
    if (group.shadowed) {
      var shadowBadge = mk('span', 'badge badge-warning', {
        ...
        title: 'All tools from this upstream conflict with another upstream (first-registered wins)'
      });
    ...
    if (group.shadowed && group.tools.length === 0) {
      ...
      shadowMsg.textContent = 'All tools from this upstream are shadowed by another upstream with the same tool names. Remove this upstream or rename the conflicting tools.';
  ...
  function openRuleModal(existingRule, policyId, prefilledToolName) {
    ...
    var varSections = [
      ...
      { category: 'Context', vars: [
        { name: 'protocol', desc: '"mcp", "http", "websocket"' },
        { name: 'framework', desc: '"crewai", "langchain", "autogen", "" ' },
        { name: 'gateway', desc: '"mcp-gateway"' },
  ...
  function buildRulesSection() {
    ...
    if (allRules.length === 0) {
      var emptyEl = mk('div', '', {
        style: 'padding: var(--space-8); text-align: center; color: var(--text-muted);'
      });
      var emptyIcon = mk('div', '', { style: 'margin-bottom: var(--space-3); opacity: 0.4;' });
      emptyIcon.innerHTML = SG.icon('shield', 32);
      emptyEl.appendChild(emptyIcon);
      var emptyTitle = mk('p', '', { style: 'font-size: var(--text-base); margin-bottom: var(--space-2);' });
      emptyTitle.textContent = 'No policy rules configured';
      emptyEl.appendChild(emptyTitle);
      var emptyHint = mk('p', '', { style: 'font-size: var(--text-sm); margin-bottom: var(--space-4);' });
      emptyHint.textContent = 'Use a template for quick setup or add rules manually.';
      emptyEl.appendChild(emptyHint);
      var emptyBtnWrap = mk('div', '', { style: 'display: flex; gap: var(--space-3); justify-content: center;' });
      var tmplBtn = mk('button', 'btn btn-primary btn-sm');
      tmplBtn.textContent = 'Use Template';
      tmplBtn.addEventListener('click', function () {
        var tmplTab = document.querySelector('[data-tab="templates"]');
        if (tmplTab) tmplTab.click();
      });
      emptyBtnWrap.appendChild(tmplBtn);
      emptyEl.appendChild(emptyBtnWrap);
  ...
  function buildTestSection() {
    ...
    sessionToggleLabel.textContent = 'Session Context (optional)';
    ...
    sessionHelpText.textContent = 'Simulate prior tool calls in the session to test session-aware rules.';
  ...
  function renderTransformRow(rule) {
    ...
    // Edit button
    var editBtn = mk('button', 'btn btn-sm btn-secondary', { title: 'Edit transform' });
    editBtn.innerHTML = SG.icon('wrench', 14);
    (function (r) {
      editBtn.addEventListener('click', function () {
    ...
    // Delete button
    var delBtn = mk('button', 'btn btn-sm btn-danger', { title: 'Delete transform' });
    delBtn.innerHTML = SG.icon('xCircle', 14);
    (function (r) {
      delBtn.addEventListener('click', function () {
  ...
  function confirmDeleteTransform(id, name) {
    SG.modal.confirm({
      ...
      confirmClass: 'btn-danger',
      onConfirm: function () {
        SG.api.del('/v1/transforms/' + id).then(function () {
  ...
  function openTransformModal(existing) {
    ...
    nameGroup.appendChild(nameLabel);
    var nameInput = mk('input', 'form-input', {
      type: 'text', name: 'tf-name', placeholder: 'e.g. redact-api-keys'
    });
    nameGroup.appendChild(nameInput);
    form.appendChild(nameGroup);
    ...
    prioHelp.textContent = 'Lower number = higher priority (applied first)';
    ...
    saveBtn.addEventListener('click', function () {
      ...
      var apiCall = isEdit
        ? SG.api.put('/v1/transforms/' + existing.id, payload)
      ...
      if (savedCheckbox.checked) {
        ...
        for (var i = 0; i < transforms.length; i++) {
          ...
          if (match === '*') {
            ...
            try {
              var re = new RegExp('^' + globToRegex(match) + '$');
              matches = re.test(toolName);
  ...
  function applyTemplate(templateId) {
    SG.api.post('/v1/templates/' + templateId + '/apply').then(function () {
      ...
      SG.modal.close();
      // Reload data to show the new policy
      var contentArea = document.querySelector('.tools-content');
      if (contentArea) {
        var pageRoot = contentArea.parentElement;
        if (pageRoot) loadData(pageRoot);
