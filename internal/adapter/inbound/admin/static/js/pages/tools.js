...
(function () {
  ...
  function renderContent(container) {
    ...
    // Empty state (tools only — Policy Rules and Policy Test are always shown below)
    if (tools.length === 0) {
   ...
   */
  function buildCELFromSimple(appliesTo, filePath) {
    var parts = [];
    if (appliesTo) {
      if (appliesTo.indexOf('*') !== -1) {
        parts.push('tool_name.matches("^' + globToRegex(appliesTo) + '$")');
      } else {
        parts.push('tool_name == "' + appliesTo + '"');
      }
    }
    if (filePath) {
      if (filePath.charAt(filePath.length - 1) === '*') {
        var prefix = filePath.slice(0, -1);
        parts.push('action_arg(arguments, "file_path").startsWith("' + prefix + '")');
      } else {
        parts.push('action_arg(arguments, "file_path") == "' + filePath + '"');
      }
    }
  ...
  function parseCELToSimple(cel) {
    if (!cel) return { appliesTo: '', filePath: '' };
    cel = cel.trim();

    // Match: true
    if (cel === 'true') {
      return { appliesTo: '', filePath: '' };
    }

    // Extract optional file_path condition
    var filePath = '';
    var toolPart = cel;

    var fileExactMatch = cel.match(/^(.+?)\s*&&\s*action_arg\(arguments,\s*"file_path"\)\s*==\s*"([^"]*)"$/);
    if (fileExactMatch) {
      toolPart = fileExactMatch[1].trim();
      filePath = fileExactMatch[2];
    } else {
      var fileStartsMatch = cel.match(/^(.+?)\s*&&\s*action_arg\(arguments,\s*"file_path"\)\.startsWith\("([^"]*)"\)$/);
      if (fileStartsMatch) {
        toolPart = fileStartsMatch[1].trim();
        filePath = fileStartsMatch[2] + '*';
      }
    }

    // Match: tool_name == "xxx"
    var eqMatch = toolPart.match(/^tool_name\s*==\s*"([^"]*)"$/);
    if (eqMatch) {
      return { appliesTo: eqMatch[1], filePath: filePath };
    }

    // Match: tool_name.matches("^xxx$")
    var matchesMatch = toolPart.match(/^tool_name\.matches\("?\^(.+)\$"?\)$/);
    if (matchesMatch) {
      var regexBody = matchesMatch[1];
      var glob = regexBody.replace(/\.\*/g, '*');
      glob = glob.replace(/\\([.+?^${}()|[\]\\])/g, '$1');
      return { appliesTo: glob, filePath: filePath };
    }
  ...
  function openRuleModal(existingRule, policyId, prefilledToolName) {
    ...
    appliesToHelp.textContent = 'Tool name sent by the agent (e.g. Read, Write, Bash, execute_query). Use * as wildcard.';
    appliesToGroup.appendChild(appliesToHelp);
    simpleContent.appendChild(appliesToGroup);

    // File Path (optional)
    var filePathGroup = mk('div', 'form-group');
    var filePathLabel = mk('label', 'form-label');
    filePathLabel.textContent = 'File Path';
    filePathLabel.setAttribute('for', 'rule-file-path');
    filePathGroup.appendChild(filePathLabel);
    var filePathInput = mk('input', 'form-input', {
      type: 'text', id: 'rule-file-path', name: 'file-path',
      placeholder: 'e.g. /etc/passwd or /secret/*'
    });
    filePathGroup.appendChild(filePathInput);
    var filePathHelp = mk('div', 'form-help');
    filePathHelp.textContent = 'Optional. Exact path or prefix with * wildcard (e.g. /secret/*)';
    filePathGroup.appendChild(filePathHelp);
    simpleContent.appendChild(filePathGroup);
    ...
    if (isEdit) {
      ...
      if (existingRule.condition) {
        ...
        if (parsed) {
          ...
          if (parsed.filePath) {
            filePathInput.value = parsed.filePath;
          }
        } else {
    ...
    advancedTab.addEventListener('click', function () {
      ...
      var celVal = celTextarea.value.trim();
      if (!celVal || celVal === lastAutoGeneratedCEL) {
        var generated = buildCELFromSimple(appliesToInput.value.trim(), filePathInput.value.trim());
    ...
    saveBtn.addEventListener('click', function () {
      ...
      if (activeRuleTab === 'simple') {
        celExpression = buildCELFromSimple(appliesToValue, filePathInput.value.trim());
      } else {
