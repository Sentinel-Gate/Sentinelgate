...
(function () {
  ...
  function render(container) {
    ...
    Promise.all([loadConfig(), loadOutboundRules(), loadOutboundStats(), loadHTTPGatewayConfig(), loadToolSecBaseline(), loadToolSecQuarantine()]).then(function (results) {
  ...
  function openRuleModal(existingRule) {
    ...
    targetsSection.innerHTML =
      ...
      '</div>' +
      '<div class="form-help" style="margin-top: var(--space-2);">Formats: domain (example.com), ip (192.168.1.1), cidr (10.0.0.0/8), domain_glob (*.example.com), port_range (8080-9090)</div>';
  ...
  function renderHTTPGatewaySection(sectionEl) {
    ...
    if (dlCaBtn) {
      dlCaBtn.addEventListener('click', function () {
        fetch('/admin/api/v1/security/http-gateway/ca-cert').then(function (resp) {
          if (!resp.ok) {
          ...
          SG.toast.error(err.message || 'Failed to download CA certificate');
        });
      });
    }
      ...
      '<div class="card security-section">' +
        ...
        '<div class="card-body" style="padding: var(--space-6);">' +
          '<p class="toolsec-info">' +
            'Manage tool baselines, detect drift from expected tool configurations, and quarantine suspicious tools.' +
          '</p>' +
            ...
            if (tools.length === 0) {
              baselineListEl.innerHTML =
                '<div class="toolsec-result-box">' +
                  '<span style="color: var(--text-muted); font-size: var(--text-sm);">No baseline captured yet. Click "Capture Baseline" to create one.</span>' +
                '</div>';
            } else {
        ...
        SG.api.get('/v1/tools/drift').then(function (result) {
          ...
          if (drifts.length === 0) {
            ...
            for (var i = 0; i < drifts.length; i++) {
              var d = drifts[i];
              var driftType = d.type || d.kind || 'unknown';
              var driftName = d.tool_name || d.name || d.tool || '';
              var typeCls = 'toolsec-drift-added';
              if (driftType === 'removed') typeCls = 'toolsec-drift-removed';
              else if (driftType === 'modified') typeCls = 'toolsec-drift-modified';
              html +=
  ...
  function renderToolSecBaselineInfo() {
    ...
    if (toolSecBaseline && toolSecBaseline.tools && toolSecBaseline.tools.length > 0) {
      ...
      infoEl.innerHTML =
        '<div style="font-size: var(--text-xs); color: var(--text-muted);">No baseline captured yet. Click "Capture Baseline" to create one.</div>';
    }
  }
