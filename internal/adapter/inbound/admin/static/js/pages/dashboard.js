...
(function () {
  ...
  function buildDashboard(container) {
    ...
    // -- Next-steps banner (UX-07) --
    var nextStepsBanner = buildNextStepsBanner();
    if (nextStepsBanner) {
      root.appendChild(nextStepsBanner);
    }
    ...
    // -- Active Sessions widget (DASH-07) --
    var sessCard = mk('div', 'card dash-enter dash-enter-5');
    var sessHeader = mk('div', 'card-header');
    var sessTitleArea = mk('span', 'card-title');
    sessTitleArea.innerHTML = SG.icon('activity', 16) + ' ';
    sessTitleArea.appendChild(document.createTextNode('Active Sessions'));
    var sessCountBadge = mk('span', 'session-count-badge');
    sessCountBadge.id = 'active-session-count';
    sessCountBadge.textContent = '0';
    sessTitleArea.appendChild(sessCountBadge);
    sessHeader.appendChild(sessTitleArea);
    sessCard.appendChild(sessHeader);
    var sessBody = mk('div', 'card-body');
    sessBody.id = 'active-sessions-container';
    var sessEmpty = mk('div', 'dist-empty-state');
    sessEmpty.id = 'sessions-empty';
    sessEmpty.textContent = 'No active sessions';
    sessBody.appendChild(sessEmpty);
  ...
  /**
   * Builds a dismissible "next steps" info banner shown after the onboarding
   * auto-dismisses (first upstream added). Uses a sessionStorage flag set by
   * onboarding.js to decide whether to display.
   *
   * Returns the banner DOM element, or null if the flag is not set.
   */
  function buildNextStepsBanner() {
    var shouldShow = false;
    try { shouldShow = sessionStorage.getItem('sg_show_nextsteps') === '1'; } catch (e) { /* private browsing */ }
    if (!shouldShow) return null;

    var banner = mk('div', 'nextsteps-banner');

    // Icon (checkCircle for success feel)
    var iconWrap = mk('div', 'nextsteps-banner-icon');
    iconWrap.innerHTML = SG.icon('checkCircle', 20);
    banner.appendChild(iconWrap);

    // Body text with links
    var body = mk('div', 'nextsteps-banner-body');

    var line1 = mk('div', '');
    var strong = mk('strong', '');
    strong.textContent = 'Your server is connected!';
    line1.appendChild(strong);
    body.appendChild(line1);

    var line2 = mk('div', '', { style: 'margin-top: 2px;' });
    line2.appendChild(document.createTextNode('Next: set up security rules in '));
    var toolsLink = mk('a', '', { href: '#/tools' });
    toolsLink.textContent = 'Tools & Rules';
    line2.appendChild(toolsLink);
    line2.appendChild(document.createTextNode(' or explore '));
    var gsLink = mk('a', '', { href: '#/getting-started' });
    gsLink.textContent = 'Getting Started';
    line2.appendChild(gsLink);
    ...
    // Close button
    var closeBtn = mk('button', 'nextsteps-banner-close', {
      'aria-label': 'Dismiss',
      title: 'Dismiss'
    });
    closeBtn.innerHTML = SG.icon('x', 16);
    closeBtn.addEventListener('click', function () {
      try { sessionStorage.removeItem('sg_show_nextsteps'); } catch (e) { /* private browsing */ }
  ...
  function buildSessionCard(sess) {
    ...
    if (sess.last_call_at) {
      ...
      var lastTime = new Date(sess.last_call_at).getTime();
      var agoSec = Math.max(0, Math.floor((Date.now() - lastTime) / 1000));
