...
(function () {
  ...
  function render(container) {
    ...
    // Template cards section (or, start with a template)
    var tmplSection = mk('div', 'onboarding-templates');
    var tmplTitle = mk('h3', 'onboarding-templates-title');
    tmplTitle.textContent = 'Quick Start with a Template';
    tmplSection.appendChild(tmplTitle);
    var tmplSubtitle = mk('p', 'onboarding-templates-subtitle');
    tmplSubtitle.textContent = 'Choose a security policy to get started instantly';
    tmplSection.appendChild(tmplSubtitle);
    var tmplGrid = mk('div', 'onboarding-template-grid');
    tmplSection.appendChild(tmplGrid);
    root.appendChild(tmplSection);

    container.innerHTML = '';
    container.appendChild(root);

    // Async-fetch templates and populate grid
    SG.api.get('/v1/templates').then(function (templateList) {
      if (!templateList || !templateList.length) return;
      for (var i = 0; i < templateList.length; i++) {
        (function (tmpl) {
          var card = mk('div', 'onboarding-template-card');

          var iconWrapEl = mk('div', 'onboarding-template-card-icon');
          iconWrapEl.innerHTML = SG.icon(tmpl.icon, 24);
          card.appendChild(iconWrapEl);

          var nameEl = mk('div', 'onboarding-template-card-name');
          nameEl.textContent = tmpl.name;
          card.appendChild(nameEl);

          var descEl = mk('div', 'onboarding-template-card-desc');
          descEl.textContent = tmpl.description;
          card.appendChild(descEl);

          if (tmpl.rules && tmpl.rules.length) {
            var badgeEl = mk('div', '', {
              style: 'display: inline-block; margin-top: var(--space-2); padding: 2px 8px; border-radius: var(--radius-full); background: var(--bg-tertiary); color: var(--text-secondary); font-size: var(--text-xs);'
            });
            badgeEl.textContent = tmpl.rules.length + ' rule' + (tmpl.rules.length !== 1 ? 's' : '');
            card.appendChild(badgeEl);
          }

          card.addEventListener('click', function () {
            SG.api.post('/v1/templates/' + tmpl.id + '/apply').then(function () {
              SG.toast.success('Policy created from template');
      ...
      // Non-fatal: templates are optional on onboarding
    });
    ...
    pollInterval = setInterval(function () {
      SG.api.get('/upstreams').then(function (upstreams) {
        if (upstreams && upstreams.length > 0) {
          cleanup();
          // Signal dashboard to show next-steps banner (UX-07)
          try { sessionStorage.setItem('sg_show_nextsteps', '1'); } catch (e) { /* private browsing */ }
