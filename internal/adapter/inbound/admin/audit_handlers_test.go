...
func (m *mockAuditReader) Query(_ context.Context, filter audit.AuditFilter) ([]audit.AuditRecord, string, error) {
...
func TestHandleAuditExport_ProtocolAndFrameworkColumns(t *testing.T) {
	...
	// Verify header includes identity_name, protocol and framework columns.
	header := rows[0]
	if len(header) < 12 {
		t.Fatalf("header columns = %d, want at least 12", len(header))
	}
	if header[3] != "identity_name" {
		t.Errorf("header[3] = %q, want 'identity_name'", header[3])
	}
	if header[10] != "protocol" {
		t.Errorf("header[10] = %q, want 'protocol'", header[10])
	}
	if header[11] != "framework" {
		t.Errorf("header[11] = %q, want 'framework'", header[11])
	}
	...
	if rows[1][10] != "mcp" {
		t.Errorf("row 1 protocol = %q, want 'mcp'", rows[1][10])
	}
	if rows[1][11] != "langchain" {
		t.Errorf("row 1 framework = %q, want 'langchain'", rows[1][11])
	}
}
