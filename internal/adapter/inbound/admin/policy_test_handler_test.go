...
// The default policy includes role-based rules and default-deny (priority 0, deny).
...
func TestHandleTestPolicy(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantDecision:   "deny",
			wantRuleID:     "default-deny",
			wantRuleName:   "default-deny",
			wantHasMatched: true,
		},
		{
			name:           "user role denied unknown tool",
		...
		{
			name:           "admin role denied write without user role",
