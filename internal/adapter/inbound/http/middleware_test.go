...
func TestDNSRebindingProtection_NoOrigin_LocalhostHost_Allowed(t *testing.T) {
	...
	// No Origin header; Host is localhost (allowed by default)
	rec := httptest.NewRecorder()
	mw(inner).ServeHTTP(rec, req)

	if !called {
		t.Error("request without Origin header and localhost Host should be allowed")
	}
...
func TestDNSRebindingProtection_NoOrigin_ForeignHost_Blocked(t *testing.T) {
	mw := DNSRebindingProtection([]string{"http://localhost:8080"})
	...
	// No Origin header but foreign Host header
	rec := httptest.NewRecorder()
	mw(inner).ServeHTTP(rec, req)

	if called {
		t.Error("request without Origin but with foreign Host should be blocked")
	}
...
func TestDNSRebindingProtection_NoOrigin_CustomAllowedHosts(t *testing.T) {
	mw := DNSRebindingProtection([]string{}, "myapp.example.com")
	...
	if !called {
		t.Error("request with allowed Host header should be allowed")
	}
	if rec.Code != http.StatusOK {
		t.Errorf("status code = %d, want %d", rec.Code, http.StatusOK)
	}
...
func TestExtractRealIP_XForwardedFor_Multiple(t *testing.T) {
	...
	ip := extractRealIP(req)
	// B9 fix: walk right-to-left, rightmost IP is inserted by the proxy and is trustworthy.
	if ip != "10.0.0.3" {
		t.Errorf("extractRealIP = %q, want rightmost IP %q", ip, "10.0.0.3")
	}
}

func TestExtractRealIP_XRealIP(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	req.RemoteAddr = "10.0.0.99:1234" // trusted private proxy
...
func TestExtractRealIP_PublicRemoteAddr_IgnoresXFF(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/", nil)
	req.RemoteAddr = "8.8.8.8:1234" // public IP, untrusted
	req.Header.Set("X-Forwarded-For", "10.0.0.1")
	req.Header.Set("X-Real-IP", "172.16.0.1")

	ip := extractRealIP(req)
	if ip != "8.8.8.8" {
		t.Errorf("extractRealIP = %q, want %q (XFF/XRI must be ignored from public IP)", ip, "8.8.8.8")
	}
}
...
func TestExtractRealIP_XForwardedFor_WithSpaces(t *testing.T) {
	...
	ip := extractRealIP(req)
	// B9 fix: walk right-to-left, rightmost IP is trustworthy.
	if ip != "10.0.0.2" {
		t.Errorf("extractRealIP = %q, want rightmost trimmed %q", ip, "10.0.0.2")
	}
}
