...
func TestRouting_CatchAllFallsToMCP(t *testing.T) {
	// All non-specific paths should reach MCP handler
	transport := newTestTransport(t)
	baseURL, cleanup := startTestServer(t, transport)
	defer cleanup()
...
func TestRouting_TableDriven(t *testing.T) {
	tests := []struct {
		...
		{"MCP explicit", "/mcp", "mcp"},
		{"MCP trailing slash", "/mcp/", "mcp"},
		{"MCP subpath", "/mcp/some/sub", "mcp"},
		{"Admin UI", "/admin/", "admin"},
		{"Admin API", "/admin/api/v1/policies", "admin"},
		{"Catch-all root", "/", "mcp"},
		{"Catch-all path", "/api/v1/data", "mcp"},
	}
