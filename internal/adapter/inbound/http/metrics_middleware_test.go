...
func TestMetricsMiddleware_RecordsRequestCount(t *testing.T) {
	...
	var m dto.Metric
	if err := metrics.RequestsTotal.WithLabelValues("POST", "ok").(prometheus.Counter).Write(&m); err != nil {
		t.Fatal(err)
	}
...
func TestMetricsMiddleware_ErrorStatus(t *testing.T) {
	...
	var m dto.Metric
	if err := metrics.RequestsTotal.WithLabelValues("POST", "error").(prometheus.Counter).Write(&m); err != nil {
		t.Fatal(err)
	}
	if m.Counter.GetValue() != 1 {
		t.Errorf("expected count 1, got %f", m.Counter.GetValue())
