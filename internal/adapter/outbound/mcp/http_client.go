...
// and returns the first JSON-RPC response (message with an "id" field).
...
func (c *HTTPClient) handleSSEResponse(body io.Reader) ([]byte, error) {
	...
	for scanner.Scan() {
		...
		case line == "":
			...
			if len(dataLines) > 0 {
				...
				// Check if this message has an "id" field (= JSON-RPC response).
				var probe struct {
				...
				// No "id" → notification/progress — consume and continue.
			}
...
// (has "method" but no "id" field). Notifications don't expect responses.
