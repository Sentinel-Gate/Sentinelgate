...
// GetAllPoliciesIncludingDisabled returns all policies regardless of enabled state.
func (s *MemoryPolicyStore) GetAllPoliciesIncludingDisabled(ctx context.Context) ([]policy.Policy, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
// Uses rejection sampling to avoid modulo bias: bytes >= maxUnbiased are
// discarded and replaced with fresh random bytes.
func randomString(n int) string {
	const letters = "abcdefghijklmnopqrstuvwxyz0123456789"
	const maxUnbiased = 252 // 36*7 = 252, largest multiple of 36 <= 256
	result := make([]byte, n)
	buf := make([]byte, n)
	filled := 0
	for filled < n {
		...
		for _, b := range buf {
			...
			result[filled] = letters[b%byte(len(letters))]
			filled++
			if filled >= n {
				break
			}
