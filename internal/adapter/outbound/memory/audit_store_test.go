...
// --- Wave 1 Tests: Filter by IdentityName ---

func TestAuditStore_FilterByIdentityName(t *testing.T) {
	t.Parallel()
	...
	now := time.Now().UTC()
	_ = store.Append(ctx,
		audit.AuditRecord{RequestID: "r1", IdentityID: "uuid-1", IdentityName: "Maxima1", ToolName: "read_file", Decision: "allow", Timestamp: now},
		audit.AuditRecord{RequestID: "r2", IdentityID: "uuid-2", IdentityName: "Bot2", ToolName: "write_file", Decision: "allow", Timestamp: now},
		audit.AuditRecord{RequestID: "r3", IdentityID: "uuid-3", IdentityName: "Maxima1", ToolName: "delete_file", Decision: "deny", Timestamp: now},
	)

	// Filter by name "Maxima1" should match 2 records (IdentityName contains)
	results, _, err := store.Query(ctx, audit.AuditFilter{UserID: "Maxima1"})
	if err != nil {
		t.Fatalf("Query error: %v", err)
	}
	if len(results) != 2 {
		t.Errorf("Query(UserID=Maxima1) returned %d records, want 2", len(results))
	}
}

func TestAuditStore_FilterByIdentityID(t *testing.T) {
	t.Parallel()
	...
	now := time.Now().UTC()
	_ = store.Append(ctx,
		audit.AuditRecord{RequestID: "r1", IdentityID: "27d7719b-abc", IdentityName: "Alice", ToolName: "read_file", Decision: "allow", Timestamp: now},
		audit.AuditRecord{RequestID: "r2", IdentityID: "other-id", IdentityName: "Bob", ToolName: "write_file", Decision: "allow", Timestamp: now},
	)

	// Exact match on IdentityID
	results, _, err := store.Query(ctx, audit.AuditFilter{UserID: "27d7719b-abc"})
	if err != nil {
		t.Fatalf("Query error: %v", err)
	}
	if len(results) != 1 {
		t.Errorf("Query(UserID=27d7719b-abc) returned %d records, want 1", len(results))
	}
	if len(results) > 0 && results[0].IdentityName != "Alice" {
		t.Errorf("IdentityName = %q, want %q", results[0].IdentityName, "Alice")
	}
}

func TestAuditStore_FilterByIdentityNameCaseInsensitive(t *testing.T) {
	t.Parallel()
	...
	// Lowercase "maxima1" should match "Maxima1" (case insensitive contains)
	results, _, err := store.Query(ctx, audit.AuditFilter{UserID: "maxima1"})
	if err != nil {
		t.Fatalf("Query error: %v", err)
	}
	if len(results) != 1 {
		t.Errorf("Query(UserID=maxima1) returned %d records, want 1", len(results))
	}
}

func TestAuditStore_FilterByIdentityNamePartialMatch(t *testing.T) {
	t.Parallel()
	...
	// Partial match "Max" should match "Maxima1" (contains)
	results, _, err := store.Query(ctx, audit.AuditFilter{UserID: "Max"})
	if err != nil {
		t.Fatalf("Query error: %v", err)
	}
	if len(results) != 1 {
		t.Errorf("Query(UserID=Max) returned %d records, want 1", len(results))
	}
}

// --- Wave 6 Tests: Audit filter with namespaced tools ---

func TestAuditStore_FilterToolMatchesNamespaced(t *testing.T) {
	t.Parallel()
	...
	now := time.Now().UTC()
	_ = store.Append(ctx,
		audit.AuditRecord{RequestID: "r1", ToolName: "desktop/read_file", Decision: "allow", Timestamp: now},
		audit.AuditRecord{RequestID: "r2", ToolName: "train/read_file", Decision: "allow", Timestamp: now},
		audit.AuditRecord{RequestID: "r3", ToolName: "desktop/write_file", Decision: "allow", Timestamp: now},
	)

	// Exact match on full namespaced name
	results, _, err := store.Query(ctx, audit.AuditFilter{ToolName: "desktop/read_file"})
	if err != nil {
		t.Fatalf("Query error: %v", err)
	}
	if len(results) != 1 {
		t.Errorf("Query(ToolName=desktop/read_file) returned %d, want 1", len(results))
	}

	// Bare name matches the bare part after "/"
	results, _, err = store.Query(ctx, audit.AuditFilter{ToolName: "read_file"})
	if err != nil {
		t.Fatalf("Query error: %v", err)
	}
	if len(results) != 2 {
		t.Errorf("Query(ToolName=read_file) returned %d, want 2 (both namespaced)", len(results))
	}
}
