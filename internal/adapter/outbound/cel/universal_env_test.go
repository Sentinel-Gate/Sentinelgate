...
// --- Wave 5 Tests: CEL session_cumulative_cost ---

func TestCEL_SessionCumulativeCost_Exists(t *testing.T) {
	// Verify the expression compiles without error
	ctx := baseMCPContext()
	ctx.SessionCumulativeCost = 75.0
	if !compileAndEval(t, `session_cumulative_cost > 50.0`, ctx) {
		t.Error("expected session_cumulative_cost > 50.0 to be true with cost=75")
	}
}

func TestCEL_SessionCumulativeCost_Evaluation(t *testing.T) {
	ctx := baseMCPContext()
	ctx.SessionCumulativeCost = 75.0

	if !compileAndEval(t, `session_cumulative_cost > 50.0`, ctx) {
		t.Error("expected session_cumulative_cost > 50.0 to be true")
	}
	if compileAndEval(t, `session_cumulative_cost > 100.0`, ctx) {
		t.Error("expected session_cumulative_cost > 100.0 to be false")
	}
}

func TestCEL_SessionCumulativeCost_Zero(t *testing.T) {
	ctx := baseMCPContext()
	ctx.SessionCumulativeCost = 0.0

	if compileAndEval(t, `session_cumulative_cost > 0.0`, ctx) {
		t.Error("expected session_cumulative_cost > 0.0 to be false when cost=0")
	}
}
