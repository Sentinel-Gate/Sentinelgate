...
func run(ctx context.Context, cfg *config.OSSConfig, statePath string, stdioTransport bool, logger *slog.Logger) error {
	...
	if cfg.HTTPGateway.Enabled {
		...
		// Wire outbound rules into CONNECT tunnel filtering.
		// Even without TLS inspection, the proxy knows the destination domain
		// from the CONNECT request and can enforce outbound blocklists.
		if ti, ok := httpGatewayHandler.(*httpgw.TLSInspector); ok {
			ti.SetConnectFilter(func(domain string, port int) error {
				rules := outboundInterceptor.GetRules()
				blocked, rule := action.EvaluateDestination(rules, domain, "", port, logger)
				if blocked && rule != nil {
					return fmt.Errorf("destination %s blocked by rule %q", domain, rule.Name)
				}
				return nil
			})
		}
