...
func run(ctx context.Context, cfg *config.OSSConfig, statePath string, stdioTransport bool, logger *slog.Logger) error {
	...
	// PostMutationHook centralizes auth store re-seeding after any identity/key mutation.
	// This prevents the D-phase production bug where manually created keys were not
	// available for authentication because ad-hoc sync was missed at certain call sites.
	identityService.PostMutationHook = func() {
		hookState, loadErr := stateStore.Load()
		if loadErr != nil {
			logger.Error("PostMutationHook: failed to load state for auth re-seed", "error", loadErr)
			return
		}
		seedAuthFromState(hookState, authStore, logger)
	}

	templateService := service.NewTemplateService(policyAdminService, logger)
	...
	apiHandler := admin.NewAdminAPIHandler(
		...
		admin.WithTemplateService(templateService),
	...
	// Create transform components for response transformation pipeline.
	// TransformInterceptor wraps ResponseScanInterceptor: transforms run AFTER
	// response scan but BEFORE the response leaves the chain toward the agent.
	transformStore := transform.NewMemoryTransformStore()
	transformExecutor := transform.NewTransformExecutor(logger)

	// Load persisted transform rules from state.json.
	for _, te := range appState.Transforms {
		...
		rule := &transform.TransformRule{
			ID:        te.ID,
			Name:      te.Name,
			Type:      transform.TransformType(te.Type),
			ToolMatch: te.ToolMatch,
		...
		_ = transformStore.Put(ctx, rule)
	}
	if len(appState.Transforms) > 0 {
		logger.Info("loaded transform rules", "count", len(appState.Transforms))
	}

	transformInterceptor := transform.NewTransformInterceptor(
		transformStore, transformExecutor, responseScanInterceptor, logger,
	)
	...
	outboundInterceptor := action.NewOutboundInterceptor(nil, dnsResolver, transformInterceptor, logger)
	...
	// ===== Session tracking (hoisted before interceptor chain for CEL session variables) =====
	// SessionTracker provides per-session call counting for quota checks and CEL policy evaluation.
	sessionTracker := session.NewSessionTracker(1*time.Minute, session.DefaultClassifier())
	...
	nativePolicyInterceptor := action.NewPolicyActionInterceptor(policyService, approvalInterceptor, logger,
		action.WithSessionUsage(&sessionUsageAdapter{tracker: sessionTracker}),
	)
	...
	// ===== Quota enforcement =====
	// Create MemoryQuotaStore and load persisted quotas from state.json.
	quotaStore := quota.NewMemoryQuotaStore()
	for _, qe := range appState.Quotas {
		cfg := &quota.QuotaConfig{
			IdentityID:           qe.IdentityID,
			MaxCallsPerSession:   qe.MaxCallsPerSession,
			MaxWritesPerSession:  qe.MaxWritesPerSession,
			MaxDeletesPerSession: qe.MaxDeletesPerSession,
			MaxCallsPerMinute:    qe.MaxCallsPerMinute,
			MaxCallsPerDay:       qe.MaxCallsPerDay,
			ToolLimits:           qe.ToolLimits,
			Action:               quota.QuotaAction(qe.Action),
			Enabled:              qe.Enabled,
		}
		_ = quotaStore.Put(ctx, cfg)
	}
	if len(appState.Quotas) > 0 {
		logger.Info("loaded quota configurations", "count", len(appState.Quotas))
	}

	quotaService := quota.NewQuotaService(quotaStore, sessionTracker)
	quotaInterceptor := quota.NewQuotaInterceptor(quotaService, sessionTracker, preAuditChain, logger)

	// Wire quota store, session tracker, and transform components into admin API handler
	// (created before BOOT-07). Setters bridge the ordering gap.
	apiHandler.SetQuotaStore(quotaStore)
	apiHandler.SetSessionTracker(sessionTracker)
	apiHandler.SetTransformStore(transformStore)
	apiHandler.SetTransformExecutor(transformExecutor)

	// ===== Session recording (passive observer, zero overhead when disabled) =====
	// Load RecordingConfig from state.json; fall back to DefaultConfig (disabled=false).
	var recordingCfg recording.RecordingConfig
	if appState.RecordingConfig != nil {
		rc := appState.RecordingConfig
		recordingCfg = recording.RecordingConfig{
			Enabled:        rc.Enabled,
			RecordPayloads: rc.RecordPayloads,
			MaxFileSize:    rc.MaxFileSize,
			RetentionDays:  rc.RetentionDays,
			RedactPatterns: rc.RedactPatterns,
			StorageDir:     rc.StorageDir,
		}
	} else {
		recordingCfg = recording.DefaultConfig()
	}
	// Resolve relative storage dir against the state file directory.
	if !filepath.IsAbs(recordingCfg.StorageDir) {
		recordingCfg.StorageDir = filepath.Join(filepath.Dir(statePath), recordingCfg.StorageDir)
	}
	if err := recordingCfg.Validate(); err != nil {
		logger.Warn("recording config invalid, using defaults", "error", err)
		recordingCfg = recording.DefaultConfig()
		recordingCfg.StorageDir = filepath.Join(filepath.Dir(statePath), recording.DefaultStorageDir)
	}

	var fileRecorder *recording.FileRecorder
	var recordingObserver *recording.RecordingObserver
	var retentionCleaner *recording.RetentionCleaner
	if fr, err := recording.NewFileRecorder(recordingCfg, logger); err != nil {
		logger.Warn("recording: failed to create FileRecorder, recording disabled", "error", err)
	} else {
		fileRecorder = fr
		// recordingSessionAdapter bridges SessionTracker to recording.SessionInfoProvider.
		recSessionAdapter := &recordingSessionAdapter{tracker: sessionTracker}
		recordingObserver = recording.NewRecordingObserver(fileRecorder, recSessionAdapter, logger)

		// Start retention cleanup goroutine.
		retentionCleaner = recording.NewRetentionCleaner(recordingCfg, logger)
		retentionCleaner.Start(ctx)

		logger.Info("session recording configured",
	...
	// Wire recording components into admin API handler.
	apiHandler.SetRecordingService(fileRecorder)
	apiHandler.SetRecordingObserver(recordingObserver)
	apiHandler.SetRetentionCleaner(retentionCleaner)

	auditInterceptor := proxy.NewAuditInterceptor(auditService, statsService, quotaInterceptor, logger)
	// Wire recording observer as async callback on the audit interceptor.
	// When enabled, each audited tool call spawns a goroutine to write to JSONL.
	if recordingObserver != nil {
		auditInterceptor.SetRecordingCallback(recordingObserver.OnAuditRecord)
	}
	...
	if cfg.HTTPGateway.Enabled {
		...
		// HTTP Gateway gets its own TransformInterceptor wrapping the response scan.
		httpGWTransform := transform.NewTransformInterceptor(
			transformStore, transformExecutor, httpGWResponseScan, logger,
		)
		httpGWOutbound := action.NewOutboundInterceptor(nil, dnsResolver, httpGWTransform, logger)
	...
	// Wire tool change notifier: broadcast notifications/tools/list_changed to SSE clients.
	toolChangeNotifier := http.NewHTTPToolChangeNotifier(transport)
	discoveryService.SetNotifier(toolChangeNotifier)
	apiHandler.SetToolChangeNotifier(toolChangeNotifier)
...
func seedAuthFromState(appState *state.AppState, authStore *memory.AuthStore, logger *slog.Logger) {
	...
	// KeyPrefix (first 8 chars of cleartext) is persisted in state.json so the
	// fast-path prefix index is restored on restart without O(n) Argon2id iteration.
	for _, key := range appState.APIKeys {
		if key.Revoked {
			continue
		}
		authStore.AddKey(&auth.APIKey{
			Key:        key.KeyHash, // Argon2id hash
			Prefix:     key.KeyPrefix, // First 8 chars of cleartext for fast-path lookup
...
// sessionUsageAdapter wraps SessionTracker to satisfy action.SessionUsageProvider
// without creating an import cycle (session -> action -> session).
...
func (a *sessionUsageAdapter) GetUsage(sessionID string) (action.SessionUsageData, bool) {
	usage, ok := a.tracker.GetUsage(sessionID)
	if !ok {
	...
	// Bridge action history from SessionTracker to action.SessionUsageData (Phase 17)
	if history, found := a.tracker.GetActionHistory(sessionID); found {
		data.ActionHistory = make([]action.SessionActionRecord, len(history))
		for i, rec := range history {
			data.ActionHistory[i] = action.SessionActionRecord{
	...
	if actionSet, found := a.tracker.GetActionSet(sessionID); found {
		data.ActionSet = actionSet
	}
	if argKeySet, found := a.tracker.GetArgKeySet(sessionID); found {
...
// recordingSessionAdapter wraps SessionTracker to satisfy recording.SessionInfoProvider
// without creating an import cycle (recording -> session -> ...).
// This mirrors the sessionUsageAdapter pattern used for CEL quota variables.
