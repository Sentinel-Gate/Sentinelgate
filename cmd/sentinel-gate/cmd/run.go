...
func runAgentInternal(args []string) (exitCode int, retErr error) {
	...
	// Step 2b: Detect Bun binary (e.g. Claude Code) for Claude hooks strategy.
	isBun := runtime.IsBunBinary(args[0])
	if isBun {
		logger.Info("detected Bun binary, will use Claude Code hooks strategy", "command", args[0])
	}
	...
	// Skip NODE_OPTIONS for Bun binaries (use PreToolUse hooks) and Gemini CLI
	...
	bootstrapCfg := runtime.BootstrapConfig{
		...
		SkipNodeOptions:    isBun || isGemini,
	...
	// Step 10b: If Bun binary detected, set up Claude Code PreToolUse hooks.
	...
	// Step 10c: For Bun binaries (e.g. Claude Code), remove the CLAUDECODE env var
