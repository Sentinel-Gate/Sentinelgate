...
export default async function globalSetup() {
  ...
  // 7b. Capture baseline and clear any auto-quarantined tools
  console.log('Capturing tool baseline...');
  await adminRequest('POST', '/v1/tools/baseline');
  // Unquarantine any auto-quarantined tools
  const quarantined = await new Promise<string>((resolve, reject) => {
    http.get('http://localhost:8080/admin/api/v1/tools/quarantine', (res) => {
      let body = '';
  ...
  try {
    const qTools = JSON.parse(quarantined);
    const toolNames = Array.isArray(qTools) ? qTools : (qTools.tools || []);
    for (const t of toolNames) {
      const name = typeof t === 'string' ? t : t.tool_name;
      if (name) {
        await adminRequest('DELETE', `/v1/tools/quarantine/${encodeURIComponent(name)}`);
      }
    }
    if (toolNames.length > 0) {
      console.log(`  Cleared ${toolNames.length} auto-quarantined tools`);
    }
  } catch { /* no quarantined tools */ }
