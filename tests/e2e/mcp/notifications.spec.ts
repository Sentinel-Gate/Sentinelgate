    ...
    const req = http.get({
      ...
      res.on('data', (chunk: Buffer) => {
        ...
        for (const part of parts) {
          ...
          // Also capture single-line data
          if (dataLines.length === 0 && part.startsWith('data: ')) {
            events.push(part.substring(6));
          }
        }
      });

      // Wait for the ": connected" comment before resolving
      // to ensure the SSE connection is fully established
      const waitForConnected = () => {
        const hasConnected = raw.some(r => r.match(/^:\s*connected/));
        if (hasConnected || raw.length > 0) {
          resolve({ events, raw, close: () => req.destroy() });
        } else {
          setTimeout(waitForConnected, 200);
        }
      };
      setTimeout(waitForConnected, 300);
    });
...
async function waitForEvents(events: string[], count: number, timeout = 20_000): Promise<void> {
...
test.describe('MCP Notifications', () => {
  ...
  test('SSE connection receives tools/list_changed on upstream add', async ({ mcpClient, adminAPI, env }) => {
    ...
    // Ensure SSE connection is fully established before triggering events
    ...
    await waitForEvents(sse.events, 1, 20_000);
  ...
  test('tools/list returns valid tools after upstream change notification', async ({ mcpClient, adminAPI, env }) => {
    ...
    // Ensure SSE connection is fully established
    await new Promise(r => setTimeout(r, 1000));
