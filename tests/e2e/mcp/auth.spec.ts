...
test.describe('MCP Auth', () => {
  ...
  test('request without API key returns auth error', async () => {
    ...
    const res = await rawMCPRequest({
      ...
      id: 1,
      method: 'initialize',
      params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'no-auth-test', version: '1.0.0' } },
    });
    ...
    const res = await rawMCPRequest(
      {
        ...
        id: 1,
        method: 'initialize',
        params: { protocolVersion: '2025-06-18', capabilities: {}, clientInfo: { name: 'bad-key-test', version: '1.0.0' } },
      },
