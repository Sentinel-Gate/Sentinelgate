...
test.describe('Access', () => {
  ...
  test('keys table visible with e2e-test-key', async ({ page }) => {
    // The keys section should contain a table with at least one row
    const keysTable = page.locator('.access-section').first().locator('table');
  ...
  test('identities section visible with e2e-tester', async ({ page, env }) => {
    ...
    await expect(identityTable).toBeVisible();
  ...
  test('create identity via API appears in UI', async ({ page, adminAPI }) => {
    ...
    try {
      ...
      await page.waitForSelector('#identities-table-container table', { timeout: 10000 });
  ...
  test('delete identity via API removes from UI', async ({ page, adminAPI }) => {
    ...
    // Confirm it exists
    await expect(collapseBody.locator('tr', { hasText: 'e2e-delete-me' })).toBeVisible();
    ...
    if (await collapseBody2.evaluate(el => el.classList.contains('collapsed')).catch(() => true)) {
  ...
  test('remove quota hides quota badge', async ({ page, adminAPI }) => {
    ...
    try {
      ...
      await expect(row).toBeVisible();
      await expect(row.locator('.quota-badge')).toBeVisible();
  ...
  test('each tab shows config snippet with server address', async ({ page }) => {
    ...
    for (let i = 0; i < tabCount; i++) {
      ...
      const snippet = activeContent.locator('.config-snippet');
