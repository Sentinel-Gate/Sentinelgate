...
// FuzzDecodeMessage tests DecodeMessage with arbitrary input.
// Must not panic on any input (A7e: fuzz testing for critical parsers).
func FuzzDecodeMessage(f *testing.F) {
	// Seed corpus with valid and malformed examples
	f.Add([]byte(`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"test"}}`))
	f.Add([]byte(`{"jsonrpc":"2.0","id":1,"result":{"content":"ok"}}`))
	f.Add([]byte(`{"jsonrpc":"2.0","id":"abc","method":"initialize"}`))
	f.Add([]byte(`{}`))
	f.Add([]byte(`{invalid`))
	f.Add([]byte(``))
	f.Add([]byte(`null`))
	f.Add([]byte(`{"jsonrpc":"2.0","id":null,"method":"test"}`))
