...
// maxResponseSize is the maximum response body size the client will read (10 MB).
const maxResponseSize = 10 << 20

// reconcileInterval is how many putInCache calls between cacheCount reconciliations.
// L-41: Prevents drift between cacheCount and actual sync.Map size.
const reconcileInterval = 50
...
type Client struct {
	...
	cacheCount     int64
	cacheMu        sync.Mutex
	reconcileCount int // L-41: Counter for periodic cacheCount reconciliation
...
func (c *Client) buildCacheKey(req EvaluateRequest) string {
	...
	if req.Arguments != nil {
		// L-1: Use fallback if json.Marshal fails instead of discarding error.
		argBytes, err := json.Marshal(req.Arguments)
		if err != nil {
			argBytes = []byte(fmt.Sprintf("%v", req.Arguments))
		}
		h.Write(argBytes)
	}
...
func (c *Client) putInCache(key string, resp *EvaluateResponse) {
	...
	// L-41: Periodically reconcile cacheCount with actual sync.Map size
	// to prevent drift from entries removed outside the mutex (e.g. in getFromCache).
	c.reconcileCount++
	if c.reconcileCount >= reconcileInterval {
		c.reconcileCount = 0
		var actual int64
