...
func TestEnvVarConfiguration(t *testing.T) {
	// Use t.Setenv which automatically restores the original value when the
	// test finishes, even if the test panics.
	t.Setenv("SENTINELGATE_SERVER_ADDR", "http://test-server:8080")
	t.Setenv("SENTINELGATE_API_KEY", "env-key-123")
	t.Setenv("SENTINELGATE_PROTOCOL", "a2a")
	t.Setenv("SENTINELGATE_FAIL_MODE", "closed")
	t.Setenv("SENTINELGATE_TIMEOUT", "10")
	t.Setenv("SENTINELGATE_CACHE_TTL", "30s")
	t.Setenv("SENTINELGATE_CACHE_MAX_SIZE", "500")
	t.Setenv("SENTINELGATE_IDENTITY_NAME", "default-agent")
	t.Setenv("SENTINELGATE_IDENTITY_ROLES", "admin,developer")
...
func TestErrorTypes(t *testing.T) {
	t.Run("PolicyDeniedError", func(t *testing.T) {
		...
		// L-2: Use strings.Contains for resilient error message checking.
		if !strings.Contains(err.Error(), "test-rule") || !strings.Contains(err.Error(), "test reason") {
	...
	t.Run("PolicyDeniedError without rule name", func(t *testing.T) {
		...
		// L-2: Use strings.Contains for resilient error message checking.
		if !strings.Contains(err.Error(), "general denial") {
			t.Errorf("unexpected error message: %s", err.Error())
		}
	})
	...
	t.Run("SentinelGateError", func(t *testing.T) {
		...
		// L-2: Use strings.Contains for resilient error message checking.
		if !strings.Contains(err.Error(), "HTTP_400") || !strings.Contains(err.Error(), "bad request") {
			t.Errorf("unexpected error message: %s", err.Error())
		}
